...
date: 1783359118 (unix) · cells: 4 · replicates: 3 · duration: 10s
...
- attribution: domains 1098908480 B vs VmRSS 1129291776 B (2.7% divergence)

| gate | threshold | measured | verdict |
|---|---|---|---|
| Pipelined GET/SET node throughput | >= 6000000 ops/s | 3561672.55 | FAIL (DEV-TIER, non-binding) |
| Unpipelined throughput vs Redis, 512 conns | >= 1.5 x Redis | 2.84 | PASS (DEV-TIER, non-binding) |
| io_uring SQEs per submit under pipelined load | >= 16 sqes/submit | 14.36 | FAIL |
| Fabric hop RTT p50 under load | < 2 us | 147.46 | FAIL (DEV-TIER, non-binding) |
| Cross-cell architecture vs Dragonfly, uniform random | >= 1.25 x Dragonfly | 1.51 | PASS (DEV-TIER, non-binding) |
| Cross-cell penalty, uniform random keys (informational; M1 S17 target) | <= 50 % vs all-local | 57.23 | FAIL (informational) |
| p99.9 latency (memtier, 8 threads) | < 3000 us | 1183.00 | PASS (DEV-TIER, non-binding) |
