...
run() { # binary scenario logname
  timeout 1800 "$1" --scenario "$2" --sweep 24 --shard 0/1 --seed 0xB44 2>&1 | grep -E "WRITE-THROUGH TICKETS|panicked|sweep shard|violations|write_through_entries_max" | head -60 > "$A/$3"
}
run $A/inf-sim-prefix-fifo m2-fua-pending pre-fix-fifo-dst-fua-pending-sweep24.log
run $A/inf-sim-prefix-fifo m2-durable     pre-fix-fifo-dst-durable-sweep24.log
