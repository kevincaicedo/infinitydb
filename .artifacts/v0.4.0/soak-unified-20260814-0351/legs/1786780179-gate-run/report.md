...
date: 1786780179 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x22b922a6de · θ 0.99
...
|---|---|
| ycsb-a-zipfian | 1.446× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.448× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 6233
combined_client p50_us = 2559 · p99_us = 155647 · p999_us = 638975 · max_us = 7040742
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xdfcee298275fda04
...
tiering_cold_resolves = 240379692
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.9285% (cold_reads 788337 · cold_resolves 1955721 — re-resolve ratio 2.48×)
  mem_hit p50_us = 2047 · p99_us = 5119 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1343 µs (client tail spread 3200 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3200 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 8359
combined_client p50_us = 1983 · p99_us = 100351 · p999_us = 688127 · max_us = 7271817
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x55d8352c5b14843c
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 243241448
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.2602% (cold_reads 966110 · cold_resolves 1679667 — re-resolve ratio 1.74×)
  mem_hit p50_us = 1599 · p99_us = 4031 · p999_us = 4223
  separation: mem_hit p99.9 4223 µs vs server cold p50 1343 µs (client tail spread 2624 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4223 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2624 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
