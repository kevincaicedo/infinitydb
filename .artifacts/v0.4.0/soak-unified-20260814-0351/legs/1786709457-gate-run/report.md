...
date: 1786709457 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x5acf34f5f · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +45.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.542× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.534× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5534
combined_client p50_us = 2623 · p99_us = 172031 · p999_us = 901119 · max_us = 4493705
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x317ac16477b54158
...
tiering_cold_resolves = 41418391
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.4120% (cold_reads 645158 · cold_resolves 1612036 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2175 · p99_us = 6399 · p999_us = 6655
  separation: mem_hit p99.9 6655 µs vs server cold p50 1311 µs (client tail spread 4480 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6655 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4480 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 7903
combined_client p50_us = 1887 · p99_us = 118783 · p999_us = 499711 · max_us = 2798766
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x3b143ae502724045
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 60415
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 43836781
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.1519% (cold_reads 623677 · cold_resolves 1264185 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1631 · p99_us = 6527 · p999_us = 7295
  separation: mem_hit p99.9 7295 µs vs server cold p50 1311 µs (client tail spread 5664 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7295 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5664 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.1
```
