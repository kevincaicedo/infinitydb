...
date: 1786796730 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x298584e179 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +35.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| ycsb-b-zipfian | 1.463× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 3846
combined_client p50_us = 4479 · p99_us = 237567 · p999_us = 1245183 · max_us = 6589020
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.27%
stream_checksum = 0xef2bc7f2ea5eae54
...
tiering_cold_resolves = 291365168
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 35.5982% (cold_reads 821464 · cold_resolves 1693662 — re-resolve ratio 2.06×)
  mem_hit p50_us = 3071 · p99_us = 5887 · p999_us = 6015
  separation: mem_hit p99.9 6015 µs vs server cold p50 1343 µs (client tail spread 2944 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6015 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2944 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6981
combined_client p50_us = 2623 · p99_us = 112639 · p999_us = 704511 · max_us = 7077682
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.28%
stream_checksum = 0x0c3669d67ad751bc
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 294513416
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 26.6357% (cold_reads 1119022 · cold_resolves 1997221 — re-resolve ratio 1.78×)
  mem_hit p50_us = 1983 · p99_us = 4031 · p999_us = 4095
  separation: mem_hit p99.9 4095 µs vs server cold p50 1343 µs (client tail spread 2112 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4095 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2112 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
