...
date: 1786741057 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x12a7804ae4 · θ 0.99
...
| ycsb-b-zipfian | 1.454× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 6334
combined_client p50_us = 2431 · p99_us = 151551 · p999_us = 933887 · max_us = 5670240
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x664d0d7ba02c1c8f
...
tiering_cold_resolves = 128883469
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 23.6075% (cold_reads 898278 · cold_resolves 1996937 — re-resolve ratio 2.22×)
  mem_hit p50_us = 1887 · p99_us = 4095 · p999_us = 4223
  separation: mem_hit p99.9 4223 µs vs server cold p50 1343 µs (client tail spread 2336 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4223 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2336 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6921
combined_client p50_us = 2495 · p99_us = 120831 · p999_us = 868351 · max_us = 7215024
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x88a74fffdf1b6405
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 131681863
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.3948% (cold_reads 888502 · cold_resolves 1541298 — re-resolve ratio 1.73×)
  mem_hit p50_us = 2015 · p99_us = 4479 · p999_us = 4607
  separation: mem_hit p99.9 4607 µs vs server cold p50 1343 µs (client tail spread 2592 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4607 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2592 µs vs cold service 1343 µs)
