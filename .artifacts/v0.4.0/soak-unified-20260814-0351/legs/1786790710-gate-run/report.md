...
date: 1786790710 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x270ca6fab5 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +16.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5592
combined_client p50_us = 2815 · p99_us = 188415 · p999_us = 917503 · max_us = 5924501
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xc2fa1e4d623552a3
...
tiering_cold_resolves = 274277765
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 23.5619% (cold_reads 790916 · cold_resolves 1976494 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2239 · p99_us = 4863 · p999_us = 4991
  separation: mem_hit p99.9 4991 µs vs server cold p50 1343 µs (client tail spread 2752 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4991 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2752 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 8377
combined_client p50_us = 1791 · p99_us = 112639 · p999_us = 466943 · max_us = 3358945
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x6b2fb093b8ae7bce
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 276860928
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.9679% (cold_reads 752464 · cold_resolves 1526150 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1503 · p99_us = 5247 · p999_us = 5631
  separation: mem_hit p99.9 5631 µs vs server cold p50 1343 µs (client tail spread 4128 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5631 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4128 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
