...
date: 1786713971 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x78799bc72 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -18.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| ycsb-b-zipfian | 1.493× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 6381
combined_client p50_us = 2175 · p99_us = 167935 · p999_us = 753663 · max_us = 4322817
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x72c7aff2c912cce8
...
tiering_cold_resolves = 53811906
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.5538% (cold_reads 748670 · cold_resolves 1869660 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1791 · p99_us = 4351 · p999_us = 4479
  separation: mem_hit p99.9 4479 µs vs server cold p50 1311 µs (client tail spread 2688 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4479 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2688 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8419
combined_client p50_us = 1631 · p99_us = 114687 · p999_us = 483327 · max_us = 5663796
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x3aa8a209c6ef9225
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 61439
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 55888296
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 12.8631% (cold_reads 649790 · cold_resolves 1315378 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1375 · p99_us = 5887 · p999_us = 6527
  separation: mem_hit p99.9 6527 µs vs server cold p50 1311 µs (client tail spread 5152 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6527 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5152 µs vs cold service 1311 µs)
