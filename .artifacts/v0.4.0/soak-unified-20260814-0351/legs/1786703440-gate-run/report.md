...
date: 1786703440 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x33415689b · θ 0.99
- mode: TIERED — the D8 refusal is lifted; rows run against the tiered namespace
- zipf self-check: top-1% share measured 72.29% vs analytic 71.87% (θ=0.99, n=20971520, 2000000 draws)
- loader: skipped (--skip-fill — an earlier leg of this campaign filled)
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +32.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.63 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 59.39 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.632× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.618× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5554
combined_client p50_us = 2751 · p99_us = 172031 · p999_us = 720895 · max_us = 4785667
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.20%
stream_checksum = 0xe4b660601fbe0811
...
tiering_cold_resolves = 25272155
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.4105% (cold_reads 647077 · cold_resolves 1616284 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2239 · p99_us = 6399 · p999_us = 6783
  separation: mem_hit p99.9 6783 µs vs server cold p50 1311 µs (client tail spread 4544 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6783 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4544 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8156
combined_client p50_us = 1759 · p99_us = 106495 · p999_us = 466943 · max_us = 3508275
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.19%
stream_checksum = 0xb6438eeddbe1fc33
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 59391
tiering_cold_p999_us (worst cell) = 360447
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 27633551
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.1296% (cold_reads 642588 · cold_resolves 1303242 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1503 · p99_us = 8063 · p999_us = 9471
  separation: mem_hit p99.9 9471 µs vs server cold p50 1311 µs (client tail spread 7968 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9471 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 7968 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.1
```
