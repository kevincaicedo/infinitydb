...
date: 1786784692 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2493c913f1 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +34.8%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.451× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5892
combined_client p50_us = 2559 · p99_us = 159743 · p999_us = 819199 · max_us = 4917785
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x565cf1b0b775367c
...
tiering_cold_resolves = 254654773
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.6927% (cold_reads 802268 · cold_resolves 1968321 — re-resolve ratio 2.45×)
  mem_hit p50_us = 2015 · p99_us = 4863 · p999_us = 5119
  separation: mem_hit p99.9 5119 µs vs server cold p50 1343 µs (client tail spread 3104 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5119 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3104 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 8678
combined_client p50_us = 1887 · p99_us = 94207 · p999_us = 622591 · max_us = 4379472
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xbe7d100c6518f9b0
tiering_cold_p50_us (worst cell) = 1343
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 257666704
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 18.7659% (cold_reads 977190 · cold_resolves 1701353 — re-resolve ratio 1.74×)
  mem_hit p50_us = 1535 · p99_us = 3903 · p999_us = 4031
  separation: mem_hit p99.9 4031 µs vs server cold p50 1343 µs (client tail spread 2496 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4031 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2496 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
