...
date: 1786753090 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x17993c186c · θ 0.99
- mode: TIERED — the D8 refusal is lifted; rows run against the tiered namespace
- zipf self-check: top-1% share measured 72.19% vs analytic 71.87% (θ=0.99, n=20971520, 2000000 draws)
- loader: skipped (--skip-fill — an earlier leg of this campaign filled)
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +40.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.448× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5476
combined_client p50_us = 2559 · p99_us = 192511 · p999_us = 802815 · max_us = 6659656
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x6519838ad790a4f9
...
tiering_cold_resolves = 164020307
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 18.5467% (cold_reads 609387 · cold_resolves 1522089 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2111 · p99_us = 6143 · p999_us = 6399
  separation: mem_hit p99.9 6399 µs vs server cold p50 1311 µs (client tail spread 4288 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6399 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4288 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 7271
combined_client p50_us = 1951 · p99_us = 135167 · p999_us = 507903 · max_us = 3491295
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x2e2a19242dd515de
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 166291889
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.1147% (cold_reads 572260 · cold_resolves 1159431 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1695 · p99_us = 8447 · p999_us = 9727
  separation: mem_hit p99.9 9727 µs vs server cold p50 1311 µs (client tail spread 8032 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9727 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8032 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.0
```
