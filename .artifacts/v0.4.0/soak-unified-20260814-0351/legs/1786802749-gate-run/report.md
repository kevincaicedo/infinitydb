...
date: 1786802749 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2bfe62c83d · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +21.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.478× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.482× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4228
combined_client p50_us = 3711 · p99_us = 212991 · p999_us = 1179647 · max_us = 7116286
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x85f4a5c2bc58d5cf
...
tiering_cold_resolves = 311399518
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 33.5133% (cold_reads 851306 · cold_resolves 1806619 — re-resolve ratio 2.12×)
  mem_hit p50_us = 2815 · p99_us = 5247 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1343 µs (client tail spread 2432 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2432 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 5390
combined_client p50_us = 2943 · p99_us = 147455 · p999_us = 884735 · max_us = 7714304
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x9146db479c4e6cb2
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 313971156
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 26.7056% (cold_reads 864715 · cold_resolves 1467411 — re-resolve ratio 1.70×)
  mem_hit p50_us = 2303 · p99_us = 4991 · p999_us = 5119
  separation: mem_hit p99.9 5119 µs vs server cold p50 1343 µs (client tail spread 2816 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5119 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2816 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
