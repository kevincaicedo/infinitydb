...
date: 1786738046 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x116b115782 · θ 0.99
...
nils = 0
ops_per_sec = 5092
combined_client p50_us = 2879 · p99_us = 192511 · p999_us = 1015807 · max_us = 5948764
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.27%
stream_checksum = 0x5f5ff58288b96f8c
...
tiering_cold_resolves = 120388716
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 25.1933% (cold_reads 769738 · cold_resolves 1870079 — re-resolve ratio 2.43×)
...
nils = 0
ops_per_sec = 7352
combined_client p50_us = 2015 · p99_us = 124927 · p999_us = 499711 · max_us = 3605929
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xb896a23a16e2ffda
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 122635608
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 15.3756% (cold_reads 678226 · cold_resolves 1375334 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1695 · p99_us = 6143 · p999_us = 6655
  separation: mem_hit p99.9 6655 µs vs server cold p50 1343 µs (client tail spread 4960 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6655 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4960 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
