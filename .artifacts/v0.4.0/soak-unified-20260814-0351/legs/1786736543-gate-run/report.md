...
date: 1786736543 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x10ccd9ddd1 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +22.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.453× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4908
combined_client p50_us = 3199 · p99_us = 184319 · p999_us = 1114111 · max_us = 5513515
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x75c29fafabb06175
...
tiering_cold_resolves = 115844227
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 31.0064% (cold_reads 913267 · cold_resolves 1943864 — re-resolve ratio 2.13×)
  mem_hit p50_us = 2431 · p99_us = 4735 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1343 µs (client tail spread 2304 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2304 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6276
combined_client p50_us = 2559 · p99_us = 135167 · p999_us = 753663 · max_us = 6058992
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xb014f64ff1cf58f6
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 118518637
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.6017% (cold_reads 851070 · cold_resolves 1453345 — re-resolve ratio 1.71×)
  mem_hit p50_us = 2047 · p99_us = 4863 · p999_us = 4991
  separation: mem_hit p99.9 4991 µs vs server cold p50 1343 µs (client tail spread 2944 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4991 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2944 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
