...
date: 1786808768 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2e7740af01 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +37.4%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.50 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.495× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4807
combined_client p50_us = 3199 · p99_us = 196607 · p999_us = 1081343 · max_us = 5818956
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.28%
stream_checksum = 0xec4853d4ef4dcdd4
...
tiering_cold_resolves = 330950155
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 31.6283% (cold_reads 912189 · cold_resolves 1937415 — re-resolve ratio 2.12×)
  mem_hit p50_us = 2303 · p99_us = 4735 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1343 µs (client tail spread 2432 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2432 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6348
combined_client p50_us = 2495 · p99_us = 126975 · p999_us = 770047 · max_us = 7518037
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x3bd4328e70bec6de
tiering_cold_p50_us (worst cell) = 1343
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 333759335
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 23.3525% (cold_reads 892605 · cold_resolves 1550094 — re-resolve ratio 1.74×)
  mem_hit p50_us = 1951 · p99_us = 4607 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1343 µs (client tail spread 2784 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2784 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
