...
date: 1786775663 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x20de7c39cb · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -7.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5513
combined_client p50_us = 3007 · p99_us = 151551 · p999_us = 1081343 · max_us = 5811315
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x05ce21eccf046a2d
...
tiering_cold_resolves = 227990852
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 29.0280% (cold_reads 960557 · cold_resolves 2064448 — re-resolve ratio 2.15×)
  mem_hit p50_us = 2367 · p99_us = 4479 · p999_us = 4607
  separation: mem_hit p99.9 4607 µs vs server cold p50 1343 µs (client tail spread 2240 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4607 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2240 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 5508
combined_client p50_us = 2879 · p99_us = 147455 · p999_us = 950271 · max_us = 5911314
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x9edccfd72ccbeb98
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 230371075
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 24.3267% (cold_reads 803889 · cold_resolves 1348806 — re-resolve ratio 1.68×)
  mem_hit p50_us = 2303 · p99_us = 5247 · p999_us = 5375
  separation: mem_hit p99.9 5375 µs vs server cold p50 1343 µs (client tail spread 3072 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5375 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3072 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
