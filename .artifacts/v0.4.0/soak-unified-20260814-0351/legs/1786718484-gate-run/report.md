...
date: 1786718484 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x962402985 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +52.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.504× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.499× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4480
combined_client p50_us = 3135 · p99_us = 237567 · p999_us = 1081343 · max_us = 3907583
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x42e02daeeffb0c7d
...
tiering_cold_resolves = 66485250
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.8456% (cold_reads 615027 · cold_resolves 1535895 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2559 · p99_us = 6143 · p999_us = 6399
  separation: mem_hit p99.9 6399 µs vs server cold p50 1311 µs (client tail spread 3840 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6399 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3840 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 7374
combined_client p50_us = 1823 · p99_us = 124927 · p999_us = 507903 · max_us = 3949069
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x06e19c8c7a8bcdd3
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 68846935
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.3804% (cold_reads 636272 · cold_resolves 1291568 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1535 · p99_us = 7167 · p999_us = 8191
  separation: mem_hit p99.9 8191 µs vs server cold p50 1343 µs (client tail spread 6656 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 8191 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 6656 µs vs cold service 1343 µs)
