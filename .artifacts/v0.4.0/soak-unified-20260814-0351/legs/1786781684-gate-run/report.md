...
date: 1786781684 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x23575a208f · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +5.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5562
combined_client p50_us = 2879 · p99_us = 155647 · p999_us = 1015807 · max_us = 8642256
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x544dab17c9f1a56a
...
tiering_cold_resolves = 245401542
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 29.8050% (cold_reads 994659 · cold_resolves 2160094 — re-resolve ratio 2.17×)
  mem_hit p50_us = 2175 · p99_us = 4351 · p999_us = 4351
  separation: mem_hit p99.9 4351 µs vs server cold p50 1343 µs (client tail spread 2176 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4351 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2176 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7114
combined_client p50_us = 2303 · p99_us = 116735 · p999_us = 753663 · max_us = 6439658
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x7b7b580528a2d2a1
tiering_cold_p50_us (worst cell) = 1343
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 248274404
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.1607% (cold_reads 947327 · cold_resolves 1694662 — re-resolve ratio 1.79×)
  mem_hit p50_us = 1823 · p99_us = 4223 · p999_us = 4351
  separation: mem_hit p99.9 4351 µs vs server cold p50 1343 µs (client tail spread 2528 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4351 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2528 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
