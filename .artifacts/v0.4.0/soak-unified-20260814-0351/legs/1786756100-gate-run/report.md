...
date: 1786756100 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x18d5ab0bce · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -9.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.44 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.439× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.444× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 6219
combined_client p50_us = 2431 · p99_us = 167935 · p999_us = 704511 · max_us = 6223066
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x640b1d7179fa3e88
...
tiering_cold_resolves = 171690805
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.0437% (cold_reads 747942 · cold_resolves 1869126 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1983 · p99_us = 5119 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1311 µs (client tail spread 3264 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3264 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 6327
combined_client p50_us = 2623 · p99_us = 131071 · p999_us = 835583 · max_us = 5472214
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.27%
stream_checksum = 0x974a4132e09b3109
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 173938083
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.1937% (cold_reads 804732 · cold_resolves 1353838 — re-resolve ratio 1.68×)
  mem_hit p50_us = 2047 · p99_us = 5247 · p999_us = 5503
  separation: mem_hit p99.9 5503 µs vs server cold p50 1311 µs (client tail spread 3456 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5503 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3456 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.0
```
