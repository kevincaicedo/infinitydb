...
date: 1786735035 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x102ea26420 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +5.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 6455
combined_client p50_us = 2431 · p99_us = 155647 · p999_us = 655359 · max_us = 6286262
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.28%
stream_checksum = 0x8e18dfcbf62397d6
...
tiering_cold_resolves = 111091256
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.9475% (cold_reads 772544 · cold_resolves 1925446 — re-resolve ratio 2.49×)
  mem_hit p50_us = 1983 · p99_us = 5119 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1343 µs (client tail spread 3264 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3264 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 8350
combined_client p50_us = 2111 · p99_us = 96255 · p999_us = 753663 · max_us = 6915541
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.28%
stream_checksum = 0x8670309d92423516
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 113900363
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.8298% (cold_reads 993739 · cold_resolves 1765956 — re-resolve ratio 1.78×)
  mem_hit p50_us = 1695 · p99_us = 3903 · p999_us = 4031
  separation: mem_hit p99.9 4031 µs vs server cold p50 1343 µs (client tail spread 2336 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4031 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2336 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
