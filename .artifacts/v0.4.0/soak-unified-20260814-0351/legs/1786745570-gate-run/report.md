...
date: 1786745570 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x148226b7f7 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +7.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5949
combined_client p50_us = 2687 · p99_us = 159743 · p999_us = 720895 · max_us = 6816451
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.29%
stream_checksum = 0x6fca8e066109c887
...
tiering_cold_resolves = 142308808
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.2381% (cold_reads 758403 · cold_resolves 1894098 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2111 · p99_us = 5503 · p999_us = 5759
  separation: mem_hit p99.9 5759 µs vs server cold p50 1343 µs (client tail spread 3648 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5759 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3648 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6754
combined_client p50_us = 2175 · p99_us = 120831 · p999_us = 589823 · max_us = 6458970
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.29%
stream_checksum = 0xc2a51aa8dea674d5
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 144594480
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.0879% (cold_reads 773646 · cold_resolves 1283722 — re-resolve ratio 1.66×)
  mem_hit p50_us = 1759 · p99_us = 5503 · p999_us = 5759
  separation: mem_hit p99.9 5759 µs vs server cold p50 1343 µs (client tail spread 4000 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5759 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4000 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
