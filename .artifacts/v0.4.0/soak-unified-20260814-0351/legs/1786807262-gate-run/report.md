...
date: 1786807262 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2dd9093550 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +10.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.492× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.495× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4899
combined_client p50_us = 3327 · p99_us = 180223 · p999_us = 1015807 · max_us = 8623496
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x892bc95276b9137b
...
tiering_cold_resolves = 326354488
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 32.2991% (cold_reads 949591 · cold_resolves 2060412 — re-resolve ratio 2.17×)
  mem_hit p50_us = 2495 · p99_us = 4863 · p999_us = 4863
  separation: mem_hit p99.9 4863 µs vs server cold p50 1343 µs (client tail spread 2368 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4863 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2368 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6025
combined_client p50_us = 2751 · p99_us = 135167 · p999_us = 983039 · max_us = 6938697
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x2ec3d25e917426df
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 329012740
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 24.3203% (cold_reads 879291 · cold_resolves 1514276 — re-resolve ratio 1.72×)
  mem_hit p50_us = 2175 · p99_us = 4607 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1343 µs (client tail spread 2560 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2560 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
