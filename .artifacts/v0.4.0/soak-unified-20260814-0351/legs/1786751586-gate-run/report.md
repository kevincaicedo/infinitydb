...
date: 1786751586 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x16fb049ebb · θ 0.99
...
nils = 0
ops_per_sec = 5931
combined_client p50_us = 2495 · p99_us = 180223 · p999_us = 671743 · max_us = 5546210
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x607c23de84abce1d
...
tiering_cold_resolves = 160032275
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.6744% (cold_reads 735878 · cold_resolves 1838081 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2015 · p99_us = 5119 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1311 µs (client tail spread 3232 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3232 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8692
combined_client p50_us = 1599 · p99_us = 110591 · p999_us = 491519 · max_us = 3440235
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x0763279c9994b44b
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 162498218
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 12.8197% (cold_reads 668627 · cold_resolves 1355075 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1343 · p99_us = 5887 · p999_us = 6527
  separation: mem_hit p99.9 6527 µs vs server cold p50 1311 µs (client tail spread 5184 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6527 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5184 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.0
```
