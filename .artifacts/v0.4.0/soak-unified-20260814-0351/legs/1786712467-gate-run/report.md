...
date: 1786712467 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x6e96242c1 · θ 0.99
...
|---|---|
| ycsb-a-zipfian | 1.511× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.506× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5394
combined_client p50_us = 2687 · p99_us = 180223 · p999_us = 720895 · max_us = 6412782
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0xc6dbdfd16d12103c
...
tiering_cold_resolves = 49594711
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.8494% (cold_reads 642379 · cold_resolves 1605581 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2111 · p99_us = 6527 · p999_us = 6911
  separation: mem_hit p99.9 6911 µs vs server cold p50 1311 µs (client tail spread 4800 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6911 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4800 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8289
combined_client p50_us = 1759 · p99_us = 110591 · p999_us = 483327 · max_us = 2543543
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0xbd2ac784d6fa26f4
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 61439
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 51942246
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.0628% (cold_reads 699896 · cold_resolves 1420977 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1471 · p99_us = 5759 · p999_us = 6271
  separation: mem_hit p99.9 6271 µs vs server cold p50 1311 µs (client tail spread 4800 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6271 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4800 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.1
```
