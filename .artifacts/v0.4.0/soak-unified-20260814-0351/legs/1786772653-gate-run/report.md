...
date: 1786772653 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x1fa20d4669 · θ 0.99
...
nils = 0
ops_per_sec = 7054
combined_client p50_us = 2303 · p99_us = 147455 · p999_us = 589823 · max_us = 3867228
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x72fd1eaef762dbfc
...
tiering_cold_resolves = 219952563
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 18.6421% (cold_reads 789273 · cold_resolves 1971750 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1887 · p99_us = 4991 · p999_us = 5119
  separation: mem_hit p99.9 5119 µs vs server cold p50 1343 µs (client tail spread 3232 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5119 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3232 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7292
combined_client p50_us = 1983 · p99_us = 135167 · p999_us = 516095 · max_us = 2867026
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x4d1e3c6c885529b0
tiering_cold_p50_us (worst cell) = 1343
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 221884643
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 11.6576% (cold_reads 510049 · cold_resolves 1029299 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1727 · p99_us = 9983 · p999_us = 11263
  separation: mem_hit p99.9 11263 µs vs server cold p50 1343 µs (client tail spread 9536 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11263 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9536 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
