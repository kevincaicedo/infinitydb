...
date: 1786769640 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x1e659e5307 · θ 0.99
...
nils = 0
ops_per_sec = 5602
combined_client p50_us = 2623 · p99_us = 188415 · p999_us = 753663 · max_us = 4799056
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x974868410d96b8a2
...
tiering_cold_resolves = 212155094
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.0143% (cold_reads 740035 · cold_resolves 1848586 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2047 · p99_us = 5119 · p999_us = 5375
  separation: mem_hit p99.9 5375 µs vs server cold p50 1343 µs (client tail spread 3328 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5375 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3328 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7070
combined_client p50_us = 2111 · p99_us = 139263 · p999_us = 483327 · max_us = 2532893
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x8ca85cea7b8688b0
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 214181493
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.4467% (cold_reads 570503 · cold_resolves 1153700 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1823 · p99_us = 8447 · p999_us = 9727
  separation: mem_hit p99.9 9727 µs vs server cold p50 1343 µs (client tail spread 7904 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9727 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 7904 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
