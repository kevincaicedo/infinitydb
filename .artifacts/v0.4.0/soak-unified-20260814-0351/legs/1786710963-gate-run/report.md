...
date: 1786710963 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x64b2ac910 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +38.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.53 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.526× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.519× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5236
combined_client p50_us = 2751 · p99_us = 200703 · p999_us = 868351 · max_us = 4441519
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0xfe27dba4d9edd778
...
tiering_cold_resolves = 45422776
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.1439% (cold_reads 634871 · cold_resolves 1585995 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2239 · p99_us = 6015 · p999_us = 6271
  separation: mem_hit p99.9 6271 µs vs server cold p50 1311 µs (client tail spread 4032 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6271 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4032 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8329
combined_client p50_us = 1791 · p99_us = 104447 · p999_us = 466943 · max_us = 3855266
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0x0e3d9b8e569f99f7
tiering_cold_p50_us (worst cell) = 1311
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 47989130
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.5067% (cold_reads 724955 · cold_resolves 1472619 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1503 · p99_us = 5759 · p999_us = 6399
  separation: mem_hit p99.9 6399 µs vs server cold p50 1311 µs (client tail spread 4896 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6399 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4896 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.1
```
