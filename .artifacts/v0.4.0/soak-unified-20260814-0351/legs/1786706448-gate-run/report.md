...
date: 1786706448 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x470845bfd · θ 0.99
- mode: TIERED — the D8 refusal is lifted; rows run against the tiered namespace
- zipf self-check: top-1% share measured 72.21% vs analytic 71.87% (θ=0.99, n=20971520, 2000000 draws)
- loader: skipped (--skip-fill — an earlier leg of this campaign filled)
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +45.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.58 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 60.41 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.582× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.573× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4604
combined_client p50_us = 3327 · p99_us = 212991 · p999_us = 1032191 · max_us = 5309091
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x593b637c45f11615
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 60415
...
tiering_cold_resolves = 33235185
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.0435% (cold_reads 553704 · cold_resolves 1383704 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2815 · p99_us = 7423 · p999_us = 7679
  separation: mem_hit p99.9 7679 µs vs server cold p50 1311 µs (client tail spread 4864 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7679 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4864 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 7064
combined_client p50_us = 2015 · p99_us = 129023 · p999_us = 540671 · max_us = 4842129
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.27%
stream_checksum = 0x4f49404a0b8f36c6
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 35499841
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.4849% (cold_reads 613979 · cold_resolves 1246760 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1727 · p99_us = 6527 · p999_us = 7295
  separation: mem_hit p99.9 7295 µs vs server cold p50 1343 µs (client tail spread 5568 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7295 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5568 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
