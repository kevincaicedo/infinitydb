...
date: 1786765126 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x1c8af7e5f4 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +38.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.450× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.453× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4957
combined_client p50_us = 2943 · p99_us = 208895 · p999_us = 901119 · max_us = 3566192
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.30%
stream_checksum = 0x5ba002b5f513ada9
...
tiering_cold_resolves = 198365732
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.8746% (cold_reads 620872 · cold_resolves 1550867 — re-resolve ratio 2.50×)
...
nils = 0
ops_per_sec = 6282
combined_client p50_us = 2431 · p99_us = 124927 · p999_us = 835583 · max_us = 6842294
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.30%
stream_checksum = 0x497c22c79927dda6
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 200790673
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.1111% (cold_reads 795933 · cold_resolves 1344831 — re-resolve ratio 1.69×)
  mem_hit p50_us = 1951 · p99_us = 5119 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1343 µs (client tail spread 3296 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3296 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
