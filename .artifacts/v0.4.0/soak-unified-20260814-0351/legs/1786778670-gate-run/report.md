...
date: 1786778670 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x221aeb2d2d · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +29.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 4772
combined_client p50_us = 2879 · p99_us = 233471 · p999_us = 999423 · max_us = 5243858
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x1d09e13dc582287d
...
tiering_cold_resolves = 236154755
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.8430% (cold_reads 653985 · cold_resolves 1633679 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2239 · p99_us = 5887 · p999_us = 6143
  separation: mem_hit p99.9 6143 µs vs server cold p50 1343 µs (client tail spread 3904 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6143 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3904 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6854
combined_client p50_us = 2175 · p99_us = 143359 · p999_us = 573439 · max_us = 4224079
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x0d2cc07c7cac6b3c
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 238423971
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.9158% (cold_reads 613575 · cold_resolves 1244188 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1791 · p99_us = 6655 · p999_us = 7167
  separation: mem_hit p99.9 7167 µs vs server cold p50 1343 µs (client tail spread 5376 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7167 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5376 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
