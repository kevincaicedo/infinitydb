...
date: 1786762118 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x1b4e88f292 · θ 0.99
...
nils = 0
ops_per_sec = 5124
combined_client p50_us = 3135 · p99_us = 184319 · p999_us = 1212415 · max_us = 6567639
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x5b93fcb7eae9607a
...
tiering_cold_resolves = 190004117
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 31.1845% (cold_reads 960135 · cold_resolves 2094052 — re-resolve ratio 2.18×)
...
nils = 0
ops_per_sec = 5443
combined_client p50_us = 2815 · p99_us = 167935 · p999_us = 786431 · max_us = 4385399
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x976fa85a9458a73e
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 192380879
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 18.7591% (cold_reads 612638 · cold_resolves 1125528 — re-resolve ratio 1.84×)
  mem_hit p50_us = 2367 · p99_us = 6655 · p999_us = 6911
  separation: mem_hit p99.9 6911 µs vs server cold p50 1343 µs (client tail spread 4544 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6911 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4544 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
