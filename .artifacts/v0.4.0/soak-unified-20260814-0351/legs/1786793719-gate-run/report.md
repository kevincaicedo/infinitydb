...
date: 1786793719 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x284915ee17 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +38.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.456× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5232
combined_client p50_us = 3007 · p99_us = 196607 · p999_us = 737279 · max_us = 5784431
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x32615d7336456da2
...
tiering_cold_resolves = 282736452
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.1710% (cold_reads 633405 · cold_resolves 1582430 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2431 · p99_us = 6399 · p999_us = 6655
  separation: mem_hit p99.9 6655 µs vs server cold p50 1343 µs (client tail spread 4224 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6655 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4224 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7511
combined_client p50_us = 1919 · p99_us = 118783 · p999_us = 491519 · max_us = 2703067
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x255b70f1007b9176
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 285069312
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.7471% (cold_reads 619679 · cold_resolves 1257025 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1599 · p99_us = 9471 · p999_us = 10495
  separation: mem_hit p99.9 10495 µs vs server cold p50 1343 µs (client tail spread 8896 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10495 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8896 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
