...
date: 1786748578 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x15be95ab59 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +28.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| ycsb-b-zipfian | 1.460× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5057
combined_client p50_us = 3071 · p99_us = 208895 · p999_us = 1048575 · max_us = 5529803
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x901c711e0f390cc7
...
tiering_cold_resolves = 151645944
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 25.1436% (cold_reads 762908 · cold_resolves 1845726 — re-resolve ratio 2.42×)
  mem_hit p50_us = 2495 · p99_us = 5119 · p999_us = 5247
  separation: mem_hit p99.9 5247 µs vs server cold p50 1311 µs (client tail spread 2752 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5247 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2752 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 6625
combined_client p50_us = 2175 · p99_us = 147455 · p999_us = 557055 · max_us = 3879453
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x6205e4eab4099cd4
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 153954743
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.9539% (cold_reads 594394 · cold_resolves 1204184 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1823 · p99_us = 7167 · p999_us = 7807
  separation: mem_hit p99.9 7807 µs vs server cold p50 1311 µs (client tail spread 5984 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7807 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5984 µs vs cold service 1311 µs)
