...
date: 1786760613 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x1ab05178e1 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +11.1%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 6040
combined_client p50_us = 2751 · p99_us = 151551 · p999_us = 950271 · max_us = 6808037
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x527bf482a60a22ea
...
tiering_cold_resolves = 185183679
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 24.6053% (cold_reads 892400 · cold_resolves 2032685 — re-resolve ratio 2.28×)
  mem_hit p50_us = 2175 · p99_us = 4479 · p999_us = 4607
  separation: mem_hit p99.9 4607 µs vs server cold p50 1343 µs (client tail spread 2432 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4607 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2432 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7324
combined_client p50_us = 2175 · p99_us = 104447 · p999_us = 802815 · max_us = 6100733
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x94297cae854fc662
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 187910065
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.4257% (cold_reads 897873 · cold_resolves 1556876 — re-resolve ratio 1.73×)
  mem_hit p50_us = 1759 · p99_us = 4351 · p999_us = 4607
  separation: mem_hit p99.9 4607 µs vs server cold p50 1343 µs (client tail spread 2848 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4607 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2848 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
