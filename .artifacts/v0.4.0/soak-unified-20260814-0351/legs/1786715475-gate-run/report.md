...
date: 1786715475 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x825d13623 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +22.1%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.497× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.505× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5354
combined_client p50_us = 2751 · p99_us = 163839 · p999_us = 835583 · max_us = 6767602
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x72d5c83d6640f140
...
tiering_cold_resolves = 57684483
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 26.5987% (cold_reads 854606 · cold_resolves 1796187 — re-resolve ratio 2.10×)
  mem_hit p50_us = 2047 · p99_us = 4863 · p999_us = 4991
  separation: mem_hit p99.9 4991 µs vs server cold p50 1311 µs (client tail spread 2944 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4991 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2944 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 6416
combined_client p50_us = 2495 · p99_us = 114687 · p999_us = 688127 · max_us = 6809268
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xfb9613a2a1b70b99
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 60376632
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 22.9472% (cold_reads 883441 · cold_resolves 1513904 — re-resolve ratio 1.71×)
  mem_hit p50_us = 1919 · p99_us = 4991 · p999_us = 5119
  separation: mem_hit p99.9 5119 µs vs server cold p50 1343 µs (client tail spread 3200 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5119 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3200 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
