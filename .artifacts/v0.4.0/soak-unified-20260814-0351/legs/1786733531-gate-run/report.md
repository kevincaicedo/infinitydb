...
date: 1786733531 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0xf906aea6f · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +30.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5017
combined_client p50_us = 2751 · p99_us = 200703 · p999_us = 917503 · max_us = 6611104
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xbbad1f685d01356e
...
tiering_cold_resolves = 107028428
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.7152% (cold_reads 593914 · cold_resolves 1483195 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2239 · p99_us = 6655 · p999_us = 6911
  separation: mem_hit p99.9 6911 µs vs server cold p50 1343 µs (client tail spread 4672 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6911 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4672 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6886
combined_client p50_us = 1823 · p99_us = 135167 · p999_us = 573439 · max_us = 4289244
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x414573cd42f12553
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 109165810
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.9083% (cold_reads 574686 · cold_resolves 1165736 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1567 · p99_us = 8959 · p999_us = 10239
  separation: mem_hit p99.9 10239 µs vs server cold p50 1343 µs (client tail spread 8672 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10239 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8672 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
