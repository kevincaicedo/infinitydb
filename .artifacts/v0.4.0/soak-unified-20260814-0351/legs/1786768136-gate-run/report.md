...
date: 1786768136 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x1dc766d956 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +14.8%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5159
combined_client p50_us = 3007 · p99_us = 192511 · p999_us = 1081343 · max_us = 4002347
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xda778c1595f269f9
...
tiering_cold_resolves = 207913628
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 26.8726% (cold_reads 831810 · cold_resolves 1938685 — re-resolve ratio 2.33×)
...
nils = 0
ops_per_sec = 7237
combined_client p50_us = 1887 · p99_us = 139263 · p999_us = 540671 · max_us = 3098374
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x2b517afae1e7b2c5
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 210306508
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 15.4570% (cold_reads 671249 · cold_resolves 1360623 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1599 · p99_us = 5759 · p999_us = 6143
  separation: mem_hit p99.9 6143 µs vs server cold p50 1343 µs (client tail spread 4544 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6143 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4544 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
