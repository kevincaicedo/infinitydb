...
date: 1786789205 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x266e6f8104 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +39.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.462× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.464× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5024
combined_client p50_us = 3391 · p99_us = 188415 · p999_us = 1146879 · max_us = 6281120
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x314d1c1e0b8ce23f
...
tiering_cold_resolves = 269458700
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 29.6244% (cold_reads 892923 · cold_resolves 1909965 — re-resolve ratio 2.14×)
  mem_hit p50_us = 2687 · p99_us = 4991 · p999_us = 5119
  separation: mem_hit p99.9 5119 µs vs server cold p50 1343 µs (client tail spread 2432 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5119 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2432 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6336
combined_client p50_us = 2367 · p99_us = 147455 · p999_us = 638975 · max_us = 4140021
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x183114af3305e7fd
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 272301271
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.1840% (cold_reads 767385 · cold_resolves 1480968 — re-resolve ratio 1.93×)
  mem_hit p50_us = 1887 · p99_us = 5119 · p999_us = 5375
  separation: mem_hit p99.9 5375 µs vs server cold p50 1343 µs (client tail spread 3488 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5375 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3488 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
