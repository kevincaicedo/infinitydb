...
date: 1786707952 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
| ycsb-b-zipfian | 1.552× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5674
combined_client p50_us = 2751 · p99_us = 188415 · p999_us = 753663 · max_us = 4281190
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x6b139b6c27fc82ab
...
tiering_cold_resolves = 37183472
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.8012% (cold_reads 674064 · cold_resolves 1683631 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2303 · p99_us = 5631 · p999_us = 5887
  separation: mem_hit p99.9 5887 µs vs server cold p50 1311 µs (client tail spread 3584 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5887 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3584 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8263
combined_client p50_us = 1887 · p99_us = 104447 · p999_us = 475135 · max_us = 3726106
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xad83e3abd2b057dc
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 39806355
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.8709% (cold_reads 688753 · cold_resolves 1398211 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1599 · p99_us = 6143 · p999_us = 6655
  separation: mem_hit p99.9 6655 µs vs server cold p50 1343 µs (client tail spread 5056 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6655 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5056 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
