...
date: 1786739550 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x120948d133 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +32.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5152
combined_client p50_us = 2559 · p99_us = 241663 · p999_us = 1081343 · max_us = 4436712
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0x670096eacb460f2e
...
tiering_cold_resolves = 124323269
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.8585% (cold_reads 675648 · cold_resolves 1687661 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2015 · p99_us = 4863 · p999_us = 4991
  separation: mem_hit p99.9 4991 µs vs server cold p50 1343 µs (client tail spread 2976 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4991 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2976 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7956
combined_client p50_us = 1887 · p99_us = 118783 · p999_us = 483327 · max_us = 3757977
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0x998ed253f4288d4b
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 126886532
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 15.0517% (cold_reads 718647 · cold_resolves 1458881 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1567 · p99_us = 5631 · p999_us = 6015
  separation: mem_hit p99.9 6015 µs vs server cold p50 1343 µs (client tail spread 4448 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6015 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4448 µs vs cold service 1343 µs)
