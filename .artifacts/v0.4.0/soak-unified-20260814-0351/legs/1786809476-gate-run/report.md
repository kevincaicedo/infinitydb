...
date: 1786809476 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 282s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2f157828b2 · θ 0.99
- mode: TIERED — the D8 refusal is lifted; rows run against the tiered namespace
- zipf self-check: top-1% share measured 72.15% vs analytic 71.87% (θ=0.99, n=20971520, 2000000 draws)
...
nils = 0
ops_per_sec = 4898
combined_client p50_us = 3199 · p99_us = 204799 · p999_us = 1146879 · max_us = 5508109
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0x6fef23e6c2633730
...
tiering_cold_resolves = 334688174
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 31.2156% (cold_reads 431369 · cold_resolves 928839 — re-resolve ratio 2.15×)
  mem_hit p50_us = 2495 · p99_us = 4607 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1343 µs (client tail spread 2240 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2240 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 6480
combined_client p50_us = 2303 · p99_us = 131071 · p999_us = 999423 · max_us = 8013776
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.19%
stream_checksum = 0x06032a0bbb5d5ffe
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 62463
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 335871099
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.8572% (cold_reads 399426 · cold_resolves 680942 — re-resolve ratio 1.70×)
  mem_hit p50_us = 1855 · p99_us = 4351 · p999_us = 4479
  separation: mem_hit p99.9 4479 µs vs server cold p50 1343 µs (client tail spread 2624 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4479 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2624 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
