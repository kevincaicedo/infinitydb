...
date: 1786704944 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s +4.7%)
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.60 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.602× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.592× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 6244
combined_client p50_us = 2239 · p99_us = 176127 · p999_us = 802815 · max_us = 3339799
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0xe36ccc9c2494493d
...
tiering_cold_resolves = 29437214
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.2751% (cold_reads 722126 · cold_resolves 1803663 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1823 · p99_us = 4863 · p999_us = 4991
  separation: mem_hit p99.9 4991 µs vs server cold p50 1311 µs (client tail spread 3168 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4991 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3168 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 8377
combined_client p50_us = 1791 · p99_us = 110591 · p999_us = 458751 · max_us = 3990558
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x08aedf83f995ca55
tiering_cold_p50_us (worst cell) = 1311
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 31851481
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.8790% (cold_reads 697720 · cold_resolves 1414947 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1503 · p99_us = 6015 · p999_us = 6527
  separation: mem_hit p99.9 6527 µs vs server cold p50 1311 µs (client tail spread 5024 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6527 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5024 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.1
```
