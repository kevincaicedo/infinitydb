...
date: 1786809598 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 46s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2fb3afa263 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -27.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.499× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5517
combined_client p50_us = 3327 · p99_us = 163839 · p999_us = 573439 · max_us = 4615792
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x2e48d6916f1a012f
...
tiering_cold_resolves = 336075760
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 33.7559% (cold_reads 91254 · cold_resolves 204661 — re-resolve ratio 2.24×)
  mem_hit p50_us = 2431 · p99_us = 4351 · p999_us = 4479
  separation: mem_hit p99.9 4479 µs vs server cold p50 1343 µs (client tail spread 2048 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4479 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2048 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7506
combined_client p50_us = 2175 · p99_us = 114687 · p999_us = 819199 · max_us = 4714935
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0x9fc89032bd8e3bbe
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 62463
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 336259404
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.4277% (cold_reads 67089 · cold_resolves 118370 — re-resolve ratio 1.76×)
  mem_hit p50_us = 1759 · p99_us = 4223 · p999_us = 4351
  separation: mem_hit p99.9 4351 µs vs server cold p50 1343 µs (client tail spread 2592 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4351 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2592 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
