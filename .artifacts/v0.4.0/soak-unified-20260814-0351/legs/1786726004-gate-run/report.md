...
date: 1786726004 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0xc795589fa · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +32.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.479× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.481× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5300
combined_client p50_us = 3199 · p99_us = 167935 · p999_us = 1114111 · max_us = 6595117
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xdedecee13e60a793
...
tiering_cold_resolves = 87830423
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 30.1460% (cold_reads 958769 · cold_resolves 2074384 — re-resolve ratio 2.16×)
  mem_hit p50_us = 2495 · p99_us = 4735 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1311 µs (client tail spread 2240 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2240 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 6005
combined_client p50_us = 2495 · p99_us = 155647 · p999_us = 622591 · max_us = 3976450
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.26%
stream_checksum = 0xf8c0b0b3fead8d52
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 61439
tiering_cold_p999_us (worst cell) = 368639
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 90344036
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 17.6101% (cold_reads 634543 · cold_resolves 1247759 — re-resolve ratio 1.97×)
  mem_hit p50_us = 2047 · p99_us = 6399 · p999_us = 6783
  separation: mem_hit p99.9 6783 µs vs server cold p50 1311 µs (client tail spread 4736 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6783 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4736 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.1
```
