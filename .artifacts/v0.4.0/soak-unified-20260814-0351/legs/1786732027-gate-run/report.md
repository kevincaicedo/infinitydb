...
date: 1786732027 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0xef23370be · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -11.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 5756
combined_client p50_us = 2303 · p99_us = 188415 · p999_us = 819199 · max_us = 3912454
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x44ca9a45a81919b0
...
tiering_cold_resolves = 103650356
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.2300% (cold_reads 665684 · cold_resolves 1662301 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1855 · p99_us = 5119 · p999_us = 5375
  separation: mem_hit p99.9 5375 µs vs server cold p50 1343 µs (client tail spread 3520 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5375 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3520 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7397
combined_client p50_us = 1791 · p99_us = 129023 · p999_us = 507903 · max_us = 3297938
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x07ed7873e2da886e
tiering_cold_p50_us (worst cell) = 1343
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 105545233
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 12.9200% (cold_reads 573838 · cold_resolves 1161970 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1535 · p99_us = 8959 · p999_us = 10239
  separation: mem_hit p99.9 10239 µs vs server cold p50 1343 µs (client tail spread 8704 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10239 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8704 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.1
```
