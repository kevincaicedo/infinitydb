...
date: 1786786196 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x2532008da2 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +17.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
nils = 0
ops_per_sec = 6028
combined_client p50_us = 2815 · p99_us = 151551 · p999_us = 1015807 · max_us = 5134116
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x09706f59c443fbb3
...
tiering_cold_resolves = 259950804
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 28.8231% (cold_reads 1042490 · cold_resolves 2284100 — re-resolve ratio 2.19×)
  mem_hit p50_us = 2111 · p99_us = 4095 · p999_us = 4223
  separation: mem_hit p99.9 4223 µs vs server cold p50 1343 µs (client tail spread 2112 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4223 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2112 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 7398
combined_client p50_us = 2303 · p99_us = 106495 · p999_us = 638975 · max_us = 7910987
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0x84d25eaa4995aead
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 263024908
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.0930% (cold_reads 936774 · cold_resolves 1699220 — re-resolve ratio 1.81×)
  mem_hit p50_us = 1855 · p99_us = 4479 · p999_us = 4607
  separation: mem_hit p99.9 4607 µs vs server cold p50 1343 µs (client tail spread 2752 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4607 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2752 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
