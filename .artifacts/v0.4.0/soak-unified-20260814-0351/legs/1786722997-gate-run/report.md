...
date: 1786722997 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0xb3ce69698 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +61.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| ycsb-b-zipfian | 1.470× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5045
combined_client p50_us = 2879 · p99_us = 212991 · p999_us = 884735 · max_us = 5876492
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.27%
stream_checksum = 0xca32f887b16262c8
...
tiering_cold_resolves = 77931957
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.4329% (cold_reads 588453 · cold_resolves 1469778 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2367 · p99_us = 6783 · p999_us = 7167
  separation: mem_hit p99.9 7167 µs vs server cold p50 1343 µs (client tail spread 4800 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7167 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4800 µs vs cold service 1343 µs)
...
nils = 0
ops_per_sec = 8218
combined_client p50_us = 1983 · p99_us = 108543 · p999_us = 483327 · max_us = 3409747
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xd38f8c2f22a5501f
tiering_cold_p50_us (worst cell) = 1343
tiering_cold_p99_us (worst cell) = 62463
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 80631184
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 14.8582% (cold_reads 732708 · cold_resolves 1489499 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1663 · p99_us = 5887 · p999_us = 6271
  separation: mem_hit p99.9 6271 µs vs server cold p50 1343 µs (client tail spread 4608 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6271 µs >= server cold p50 1343 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4608 µs vs cold service 1343 µs)
tripwire sqes/submit = 6.0
```
