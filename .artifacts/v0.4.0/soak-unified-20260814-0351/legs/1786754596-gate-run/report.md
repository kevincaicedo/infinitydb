...
date: 1786754596 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0x183773921d · θ 0.99
...
| ycsb-b-zipfian | 1.441× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 5166
combined_client p50_us = 2879 · p99_us = 212991 · p999_us = 933887 · max_us = 3374261
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0xdfd6a65c6b76c997
...
tiering_cold_resolves = 167816656
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 19.6624% (cold_reads 610589 · cold_resolves 1524767 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2367 · p99_us = 6143 · p999_us = 6399
  separation: mem_hit p99.9 6399 µs vs server cold p50 1311 µs (client tail spread 4032 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6399 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4032 µs vs cold service 1311 µs)
...
nils = 0
ops_per_sec = 6262
combined_client p50_us = 2175 · p99_us = 159743 · p999_us = 524287 · max_us = 4776737
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0x5c9ea0a17538f0da
tiering_cold_p50_us (worst cell) = 1311
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 376831
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 169821679
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 13.2026% (cold_reads 496362 · cold_resolves 1004775 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1855 · p99_us = 11775 · p999_us = 13311
  separation: mem_hit p99.9 13311 µs vs server cold p50 1311 µs (client tail spread 11456 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13311 µs >= server cold p50 1311 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11456 µs vs cold service 1311 µs)
tripwire sqes/submit = 6.0
```
