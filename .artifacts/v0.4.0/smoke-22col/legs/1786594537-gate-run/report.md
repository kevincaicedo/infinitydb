...
date: 1786594537 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 20s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 655360 keys × 1024 B = 640 MiB = 10× the 64 MiB memory budget · seed 0x1f7a67539 · θ 0.99
- mode: TIERED — the D8 refusal is lifted; rows run against the tiered namespace
- zipf self-check: top-1% share measured 66.07% vs analytic 65.48% (θ=0.99, n=655360, 2000000 draws)
- loader: skipped (--skip-fill — an earlier leg of this campaign filled)
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -13.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.61 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 24.06 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.608× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.589× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 17843
combined_client p50_us = 2367 · p99_us = 26623 · p999_us = 188415 · max_us = 926740
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 66.06%
stream_checksum = 0xfbf7068e9ac6152f
tiering_cold_p50_us (worst cell) = 1151
tiering_cold_p99_us (worst cell) = 24063
...
tiering_cold_resolves = 1286116
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 23.8102% (cold_reads 85354 · cold_resolves 213486 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1887 · p99_us = 3455 · p999_us = 3455
  separation: mem_hit p99.9 3455 µs vs server cold p50 1151 µs (client tail spread 1568 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 3455 µs >= server cold p50 1151 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 1568 µs vs cold service 1151 µs)
...
nils = 0
ops_per_sec = 24497
combined_client p50_us = 1695 · p99_us = 18943 · p999_us = 90111 · max_us = 894706
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 66.11%
stream_checksum = 0x6c18ecc45677f603
tiering_cold_p50_us (worst cell) = 1151
tiering_cold_p99_us (worst cell) = 23551
tiering_cold_p999_us (worst cell) = 192511
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 1542306
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 16.8871% (cold_reads 84211 · cold_resolves 171091 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1407 · p99_us = 3007 · p999_us = 3071
  separation: mem_hit p99.9 3071 µs vs server cold p50 1151 µs (client tail spread 1664 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 3071 µs >= server cold p50 1151 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 1664 µs vs cold service 1151 µs)
