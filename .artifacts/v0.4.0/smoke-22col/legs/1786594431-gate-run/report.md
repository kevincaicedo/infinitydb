...
date: 1786594431 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 20s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 655360 keys × 1024 B = 640 MiB = 10× the 64 MiB memory budget · seed 0xbb3781d7 · θ 0.99
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.78 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.782× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.741× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 18064
combined_client p50_us = 2303 · p99_us = 25599 · p999_us = 172031 · max_us = 610421
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 66.00%
stream_checksum = 0xa4f0d997693acee1
tiering_cold_p50_us (worst cell) = 1183
tiering_cold_p99_us (worst cell) = 41983
tiering_cold_p999_us (worst cell) = 278527
...
tiering_cold_resolves = 411937
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 23.9055% (cold_reads 86374 · cold_resolves 216098 — re-resolve ratio 2.50×)
  mem_hit p50_us = 1855 · p99_us = 3519 · p999_us = 3583
  separation: mem_hit p99.9 3583 µs vs server cold p50 1183 µs (client tail spread 1728 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 3583 µs >= server cold p50 1183 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 1728 µs vs cold service 1183 µs)
...
nils = 0
ops_per_sec = 24206
combined_client p50_us = 1823 · p99_us = 18943 · p999_us = 83967 · max_us = 343151
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 66.02%
stream_checksum = 0x73ac911013185b65
tiering_cold_p50_us (worst cell) = 1151
tiering_cold_p99_us (worst cell) = 27647
tiering_cold_p999_us (worst cell) = 245759
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 646900
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 16.1209% (cold_reads 78069 · cold_resolves 158512 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1567 · p99_us = 3391 · p999_us = 3455
  separation: mem_hit p99.9 3455 µs vs server cold p50 1151 µs (client tail spread 1888 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 3455 µs >= server cold p50 1151 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 1888 µs vs cold service 1151 µs)
