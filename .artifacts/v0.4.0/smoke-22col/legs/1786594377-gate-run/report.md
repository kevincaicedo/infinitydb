...
date: 1786594377 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 20s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +83.1%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 184.32 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.903× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.880× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 1967
combined_client p50_us = 3327 · p99_us = 516095 · p999_us = 2949119 · max_us = 3794046
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 65.61%
stream_checksum = 0xdf6bc13bf0ddf1a4
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 184319
tiering_cold_p999_us (worst cell) = 933887
...
tiering_cold_resolves = 55475
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 56.4423% (cold_reads 22214 · cold_resolves 55475 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2047 · p99_us = 2943 · p999_us = 3007
  separation: mem_hit p99.9 3007 µs vs server cold p50 1247 µs (client tail spread 960 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — 39357 ops < 100000 — too few for a p99.9 gate value
...
nils = 0
ops_per_sec = 7538
combined_client p50_us = 2495 · p99_us = 159743 · p999_us = 442367 · max_us = 744265
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 65.97%
stream_checksum = 0x7b60f107d0b9119c
tiering_cold_p50_us (worst cell) = 1215
tiering_cold_p99_us (worst cell) = 71679
tiering_cold_p999_us (worst cell) = 393215
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 195839
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 29.5001% (cold_reads 45070 · cold_resolves 91926 — re-resolve ratio 2.04×)
  mem_hit p50_us = 1887 · p99_us = 3455 · p999_us = 3519
  separation: mem_hit p99.9 3519 µs vs server cold p50 1215 µs (client tail spread 1632 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 3519 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 1632 µs vs cold service 1215 µs)
