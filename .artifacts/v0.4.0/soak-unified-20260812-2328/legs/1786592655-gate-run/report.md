...
date: 1786592655 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 80s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- dataset: 20971520 keys × 1024 B = 20480 MiB = 10× the 2048 MiB memory budget · seed 0xbb3781d7 · θ 0.99
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +25.4%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.92 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 126.97 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.916× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.914× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 2490
combined_client p50_us = 4607 · p99_us = 417791 · p999_us = 2162687 · max_us = 4352882
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.11%
stream_checksum = 0x1251a505ca6d7448
tiering_cold_p50_us (worst cell) = 1535
tiering_cold_p99_us (worst cell) = 126975
tiering_cold_p999_us (worst cell) = 540671
...
tiering_cold_resolves = 631652
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 38.6219% (cold_reads 77291 · cold_resolves 193117 — re-resolve ratio 2.50×)
  mem_hit p50_us = 3135 · p99_us = 6015 · p999_us = 6143
  separation: mem_hit p99.9 6143 µs vs server cold p50 1535 µs (client tail spread 3008 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6143 µs >= server cold p50 1535 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3008 µs vs cold service 1535 µs)
tripwire sqes/submit = 5.5
```
...
nils = 0
ops_per_sec = 4425
combined_client p50_us = 4351 · p99_us = 212991 · p999_us = 622591 · max_us = 2170066
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.17%
stream_checksum = 0xda45cf57ca570a5b
tiering_cold_p50_us (worst cell) = 1503
tiering_cold_p99_us (worst cell) = 112639
tiering_cold_p999_us (worst cell) = 491519
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 932651
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 25.4648% (cold_reads 90541 · cold_resolves 183862 — re-resolve ratio 2.03×)
  mem_hit p50_us = 3327 · p99_us = 7423 · p999_us = 7679
  separation: mem_hit p99.9 7679 µs vs server cold p50 1503 µs (client tail spread 4352 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7679 µs >= server cold p50 1503 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4352 µs vs cold service 1503 µs)
tripwire sqes/submit = 5.5
```
