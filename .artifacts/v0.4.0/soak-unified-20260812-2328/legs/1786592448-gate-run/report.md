...
date: 1786592448 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 80s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -69.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 120.83 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.918× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.917× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 2532
combined_client p50_us = 4735 · p99_us = 434175 · p999_us = 1671167 · max_us = 2095753
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.18%
stream_checksum = 0xf4050b96b76b6685
tiering_cold_p50_us (worst cell) = 1567
tiering_cold_p99_us (worst cell) = 110591
tiering_cold_p999_us (worst cell) = 573439
...
tiering_cold_resolves = 235006
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 46.3494% (cold_reads 94106 · cold_resolves 234886 — re-resolve ratio 2.50×)
  mem_hit p50_us = 3007 · p99_us = 4991 · p999_us = 4991
  separation: mem_hit p99.9 4991 µs vs server cold p50 1567 µs (client tail spread 1984 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4991 µs >= server cold p50 1567 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 1984 µs vs cold service 1567 µs)
...
nils = 0
ops_per_sec = 3864
combined_client p50_us = 3455 · p99_us = 258047 · p999_us = 1310719 · max_us = 2828568
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.20%
stream_checksum = 0x3e23786e556dd2c8
tiering_cold_p50_us (worst cell) = 1535
tiering_cold_p99_us (worst cell) = 120831
tiering_cold_p999_us (worst cell) = 540671
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 438535
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 27.1379% (cold_reads 84031 · cold_resolves 170186 — re-resolve ratio 2.03×)
  mem_hit p50_us = 2751 · p99_us = 6271 · p999_us = 6399
  separation: mem_hit p99.9 6399 µs vs server cold p50 1535 µs (client tail spread 3648 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6399 µs >= server cold p50 1535 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3648 µs vs cold service 1535 µs)
tripwire sqes/submit = 5.6
```
