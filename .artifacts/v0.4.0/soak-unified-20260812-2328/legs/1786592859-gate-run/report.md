...
date: 1786592859 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 80s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +36.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.91 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 112.64 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.912× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.909× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
tiering_cold_resolves = 1109410
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 34.6907% (cold_reads 70855 · cold_resolves 176759 — re-resolve ratio 2.49×)
  mem_hit p50_us = 3263 · p99_us = 6783 · p999_us = 6911
  separation: mem_hit p99.9 6911 µs vs server cold p50 1503 µs (client tail spread 3648 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6911 µs >= server cold p50 1503 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3648 µs vs cold service 1503 µs)
...
nils = 0
ops_per_sec = 5889
combined_client p50_us = 2943 · p99_us = 184319 · p999_us = 638975 · max_us = 3024128
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.19%
stream_checksum = 0xc7f6311f79f6fca0
tiering_cold_p50_us (worst cell) = 1503
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 1483423
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 25.7863% (cold_reads 121518 · cold_resolves 247460 — re-resolve ratio 2.04×)
  mem_hit p50_us = 2239 · p99_us = 4735 · p999_us = 4863
  separation: mem_hit p99.9 4863 µs vs server cold p50 1503 µs (client tail spread 2624 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4863 µs >= server cold p50 1503 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2624 µs vs cold service 1503 µs)
