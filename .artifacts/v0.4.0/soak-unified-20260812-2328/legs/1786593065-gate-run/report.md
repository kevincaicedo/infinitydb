...
date: 1786593065 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 80s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -46.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.906× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.904× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 3621
combined_client p50_us = 3903 · p99_us = 237567 · p999_us = 950271 · max_us = 3402371
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.21%
stream_checksum = 0xab1434630050bdc8
...
tiering_cold_resolves = 1719099
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 32.4729% (cold_reads 94264 · cold_resolves 235676 — re-resolve ratio 2.50×)
  mem_hit p50_us = 2943 · p99_us = 6143 · p999_us = 6271
  separation: mem_hit p99.9 6271 µs vs server cold p50 1535 µs (client tail spread 3328 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6271 µs >= server cold p50 1535 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3328 µs vs cold service 1535 µs)
...
nils = 0
ops_per_sec = 6091
combined_client p50_us = 2623 · p99_us = 159743 · p999_us = 540671 · max_us = 3304332
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xb23d37da55d6d7da
tiering_cold_p50_us (worst cell) = 1535
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 2001432
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.9449% (cold_reads 107091 · cold_resolves 217300 — re-resolve ratio 2.03×)
  mem_hit p50_us = 2111 · p99_us = 4991 · p999_us = 5119
  separation: mem_hit p99.9 5119 µs vs server cold p50 1535 µs (client tail spread 3008 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5119 µs >= server cold p50 1535 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3008 µs vs cold service 1535 µs)
