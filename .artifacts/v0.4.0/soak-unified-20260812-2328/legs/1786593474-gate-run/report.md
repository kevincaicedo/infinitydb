...
date: 1786593474 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 80s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +44.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 104.45 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.897× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.894× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 3359
combined_client p50_us = 4223 · p99_us = 278527 · p999_us = 1114111 · max_us = 3205760
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.08%
stream_checksum = 0xb3d5b08cb2b0c4e4
tiering_cold_p50_us (worst cell) = 1503
tiering_cold_p99_us (worst cell) = 104447
tiering_cold_p999_us (worst cell) = 450559
...
tiering_cold_resolves = 2722996
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 29.8769% (cold_reads 80293 · cold_resolves 200576 — re-resolve ratio 2.50×)
  mem_hit p50_us = 3135 · p99_us = 7039 · p999_us = 7295
  separation: mem_hit p99.9 7295 µs vs server cold p50 1503 µs (client tail spread 4160 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7295 µs >= server cold p50 1503 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4160 µs vs cold service 1503 µs)
...
nils = 0
ops_per_sec = 5979
combined_client p50_us = 2559 · p99_us = 159743 · p999_us = 540671 · max_us = 2223004
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.14%
stream_checksum = 0x02767047d23dfc5a
tiering_cold_p50_us (worst cell) = 1503
tiering_cold_p99_us (worst cell) = 100351
tiering_cold_p999_us (worst cell) = 442367
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 3062575
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 20.3854% (cold_reads 97968 · cold_resolves 198865 — re-resolve ratio 2.03×)
  mem_hit p50_us = 2015 · p99_us = 5887 · p999_us = 6143
  separation: mem_hit p99.9 6143 µs vs server cold p50 1503 µs (client tail spread 4128 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 6143 µs >= server cold p50 1503 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 4128 µs vs cold service 1503 µs)
tripwire sqes/submit = 5.8
```
