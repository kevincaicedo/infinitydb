...
date: 1786593883 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 80s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +36.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 98.30 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.881× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.878× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 4145
combined_client p50_us = 3327 · p99_us = 262143 · p999_us = 983039 · max_us = 2550883
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.35%
stream_checksum = 0xd8fdbb9e99593af4
tiering_cold_p50_us (worst cell) = 1503
tiering_cold_p99_us (worst cell) = 98303
...
tiering_cold_resolves = 3919678
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 26.5237% (cold_reads 88271 · cold_resolves 220616 — re-resolve ratio 2.50×)
...
nils = 0
ops_per_sec = 6915
combined_client p50_us = 2367 · p99_us = 139263 · p999_us = 442367 · max_us = 2339087
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.33%
stream_checksum = 0x1489a940cc8b4886
tiering_cold_p50_us (worst cell) = 1503
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 4277444
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 18.6320% (cold_reads 103272 · cold_resolves 209562 — re-resolve ratio 2.03×)
  mem_hit p50_us = 1919 · p99_us = 5503 · p999_us = 5759
  separation: mem_hit p99.9 5759 µs vs server cold p50 1503 µs (client tail spread 3840 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 5759 µs >= server cold p50 1503 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3840 µs vs cold service 1503 µs)
tripwire sqes/submit = 5.7
```
