...
date: 1786488678 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -17.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 43.01 | FAIL (DEV-TIER, non-binding) |
...
| ycsb-b-zipfian | 1.440× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8900
combined_client p50_us = 1247 · p99_us = 120831 · p999_us = 516095 · max_us = 3743582
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x217f9c85bb55f65c
...
tiering_cold_resolves = 30915071
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.4727% (cold_reads 505989 · cold_resolves 1042901 — re-resolve ratio 2.06×)
  mem_hit p50_us = 1087 · p99_us = 8959 · p999_us = 10751
  separation: mem_hit p99.9 10751 µs vs server cold p50 1215 µs (client tail spread 9664 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10751 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9664 µs vs cold service 1215 µs)
...
nils = 0
ops_per_sec = 9892
combined_client p50_us = 1279 · p99_us = 98303 · p999_us = 450559 · max_us = 3375885
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xbac43a4ae4e1d779
tiering_cold_p50_us (worst cell) = 1215
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 32662967
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.6143% (cold_reads 570632 · cold_resolves 1142355 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 8703 · p999_us = 10495
  separation: mem_hit p99.9 10495 µs vs server cold p50 1215 µs (client tail spread 9408 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10495 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9408 µs vs cold service 1215 µs)
tripwire sqes/submit = 6.0
```
