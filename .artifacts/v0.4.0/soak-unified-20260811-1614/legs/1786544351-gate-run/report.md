...
date: 1786544351 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +26.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.127× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.126× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8653
combined_client p50_us = 1247 · p99_us = 120831 · p999_us = 540671 · max_us = 3933289
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xd8b858e4a0919863
...
tiering_cold_resolves = 128482428
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6711% (cold_reads 450597 · cold_resolves 901196 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 13311 · p999_us = 15871
  separation: mem_hit p99.9 15871 µs vs server cold p50 1279 µs (client tail spread 14816 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15871 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14816 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 11163
combined_client p50_us = 1215 · p99_us = 81919 · p999_us = 401407 · max_us = 3110376
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xffc00180ab5cfc6e
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 130272146
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.0012% (cold_reads 603071 · cold_resolves 1206614 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 8191 · p999_us = 10239
  separation: mem_hit p99.9 10239 µs vs server cold p50 1279 µs (client tail spread 9152 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10239 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9152 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
