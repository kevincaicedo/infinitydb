...
date: 1786580469 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +12.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.091× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.091× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9556
combined_client p50_us = 1151 · p99_us = 108543 · p999_us = 483327 · max_us = 3635070
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x884b67ffe716bce4
...
tiering_cold_resolves = 188790391
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7155% (cold_reads 499756 · cold_resolves 999552 — re-resolve ratio 2.00×)
...
nils = 0
ops_per_sec = 10424
combined_client p50_us = 1183 · p99_us = 88063 · p999_us = 425983 · max_us = 3914107
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x8e692754efc8a108
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 190440364
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7225% (cold_reads 545805 · cold_resolves 1091616 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 10751 · p999_us = 12799
  separation: mem_hit p99.9 12799 µs vs server cold p50 1279 µs (client tail spread 11776 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12799 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11776 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
