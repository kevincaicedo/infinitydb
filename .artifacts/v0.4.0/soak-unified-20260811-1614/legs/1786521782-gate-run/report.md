...
date: 1786521782 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +51.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.18 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.176× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.174× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8246
combined_client p50_us = 1343 · p99_us = 126975 · p999_us = 540671 · max_us = 3371930
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x211ce61cbe0564db
...
tiering_cold_resolves = 89246755
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7439% (cold_reads 432619 · cold_resolves 865954 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1151 · p99_us = 15615 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1247 µs (client tail spread 15744 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15744 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9628
combined_client p50_us = 1151 · p99_us = 106495 · p999_us = 466943 · max_us = 3552100
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xc166d03faaeffc20
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 90928059
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7420% (cold_reads 505175 · cold_resolves 1010358 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1007 · p99_us = 11007 · p999_us = 13311
  separation: mem_hit p99.9 13311 µs vs server cold p50 1247 µs (client tail spread 12304 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13311 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12304 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
