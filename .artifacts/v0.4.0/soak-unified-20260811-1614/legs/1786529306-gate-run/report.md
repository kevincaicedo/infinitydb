...
date: 1786529306 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -5.4%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.155× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.154× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10011
combined_client p50_us = 1279 · p99_us = 92159 · p999_us = 466943 · max_us = 3828632
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x62a57e7b87681ddf
...
tiering_cold_resolves = 102756768
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7174% (cold_reads 523633 · cold_resolves 1049143 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1119 · p99_us = 9727 · p999_us = 11519
  separation: mem_hit p99.9 11519 µs vs server cold p50 1247 µs (client tail spread 10400 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11519 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10400 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9737
combined_client p50_us = 1215 · p99_us = 98303 · p999_us = 466943 · max_us = 4365678
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xb519390e8017fa5c
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 104277770
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7172% (cold_reads 509269 · cold_resolves 1018540 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 11519 · p999_us = 14079
  separation: mem_hit p99.9 14079 µs vs server cold p50 1247 µs (client tail spread 13024 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14079 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13024 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
