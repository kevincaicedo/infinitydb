...
date: 1786491686 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s -1.5%)
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.40 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 47.10 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.398× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.389× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9510
combined_client p50_us = 1279 · p99_us = 106495 · p999_us = 475135 · max_us = 4200932
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x738497eb8e56f463
tiering_cold_p50_us (worst cell) = 1215
tiering_cold_p99_us (worst cell) = 45055
tiering_cold_p999_us (worst cell) = 303103
...
tiering_cold_resolves = 36279475
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.3498% (cold_reads 533572 · cold_resolves 1096826 — re-resolve ratio 2.06×)
  mem_hit p50_us = 1119 · p99_us = 8191 · p999_us = 10239
  separation: mem_hit p99.9 10239 µs vs server cold p50 1215 µs (client tail spread 9120 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10239 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9120 µs vs cold service 1215 µs)
...
nils = 0
ops_per_sec = 8670
combined_client p50_us = 1279 · p99_us = 108543 · p999_us = 475135 · max_us = 3313475
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x7afb4c78bac20a22
tiering_cold_p50_us (worst cell) = 1215
tiering_cold_p99_us (worst cell) = 47103
tiering_cold_p999_us (worst cell) = 303103
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 37833256
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.9937% (cold_reads 467831 · cold_resolves 936598 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 16127 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1215 µs (client tail spread 16320 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16320 µs vs cold service 1215 µs)
tripwire sqes/submit = 6.0
```
