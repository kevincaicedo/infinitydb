...
date: 1786512757 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +26.8%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.21 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.211× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.208× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9642
combined_client p50_us = 1215 · p99_us = 104447 · p999_us = 491519 · max_us = 4437947
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x5efaf070bc5f713e
...
tiering_cold_resolves = 72915634
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8388% (cold_reads 511394 · cold_resolves 1029791 — re-resolve ratio 2.01×)
...
nils = 0
ops_per_sec = 10146
combined_client p50_us = 1215 · p99_us = 90111 · p999_us = 409599 · max_us = 2794766
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xeaecab787913b41f
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 62463
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 74624127
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7166% (cold_reads 530657 · cold_resolves 1061324 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 11775 · p999_us = 14335
  separation: mem_hit p99.9 14335 µs vs server cold p50 1247 µs (client tail spread 13280 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14335 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13280 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
