...
date: 1786509749 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +54.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.23 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.227× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.224× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 7964
combined_client p50_us = 1407 · p99_us = 129023 · p999_us = 540671 · max_us = 3717901
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x6f1aed51621264bb
...
tiering_cold_resolves = 67687721
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7892% (cold_reads 419989 · cold_resolves 839980 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 14847 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1247 µs (client tail spread 15648 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15648 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9257
combined_client p50_us = 1151 · p99_us = 106495 · p999_us = 466943 · max_us = 4194516
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xa86627613071a0d2
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 69329804
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7299% (cold_reads 484902 · cold_resolves 969807 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1007 · p99_us = 12799 · p999_us = 15615
  separation: mem_hit p99.9 15615 µs vs server cold p50 1247 µs (client tail spread 14608 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15615 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14608 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
