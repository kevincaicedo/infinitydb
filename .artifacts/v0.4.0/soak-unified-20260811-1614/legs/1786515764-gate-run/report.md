...
date: 1786515764 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -9.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.20 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.197× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.195× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9636
combined_client p50_us = 1247 · p99_us = 102399 · p999_us = 458751 · max_us = 4681193
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x1964280e0648b9fe
...
tiering_cold_resolves = 78627752
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.9531% (cold_reads 517636 · cold_resolves 1046025 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1087 · p99_us = 11263 · p999_us = 13311
  separation: mem_hit p99.9 13311 µs vs server cold p50 1247 µs (client tail spread 12224 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13311 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12224 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 8498
combined_client p50_us = 1407 · p99_us = 122879 · p999_us = 491519 · max_us = 3625017
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xe5359cf682291f02
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 79971601
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7058% (cold_reads 443871 · cold_resolves 887746 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 14335 · p999_us = 16895
