...
date: 1786542846 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +44.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.13 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.129× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.128× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 7871
combined_client p50_us = 1407 · p99_us = 131071 · p999_us = 540671 · max_us = 3686392
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0xd400966bcf2585a7
...
tiering_cold_resolves = 126033550
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7507% (cold_reads 413278 · cold_resolves 826562 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 14591 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1279 µs (client tail spread 15648 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15648 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9154
combined_client p50_us = 1471 · p99_us = 106495 · p999_us = 450559 · max_us = 3441832
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xf717e8ac56ec3125
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 127581232
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6785% (cold_reads 477014 · cold_resolves 954030 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1311 · p99_us = 12031 · p999_us = 14847
  separation: mem_hit p99.9 14847 µs vs server cold p50 1279 µs (client tail spread 13536 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14847 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13536 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
