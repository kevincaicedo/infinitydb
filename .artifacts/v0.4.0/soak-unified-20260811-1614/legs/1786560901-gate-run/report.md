...
date: 1786560901 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
|---|---|
| ycsb-a-zipfian | 1.107× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.107× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9361
combined_client p50_us = 1247 · p99_us = 102399 · p999_us = 442367 · max_us = 3397815
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xa49fb39fd31ad836
...
tiering_cold_resolves = 155875341
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6577% (cold_reads 486303 · cold_resolves 972701 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 14079 · p999_us = 16127
  separation: mem_hit p99.9 16127 µs vs server cold p50 1279 µs (client tail spread 15040 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16127 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15040 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 8746
combined_client p50_us = 1279 · p99_us = 116735 · p999_us = 499711 · max_us = 4077672
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x1e7ebadd8b7fb8fb
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 157395756
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6657% (cold_reads 454912 · cold_resolves 909826 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1119 · p99_us = 12031 · p999_us = 14847
  separation: mem_hit p99.9 14847 µs vs server cold p50 1279 µs (client tail spread 13728 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14847 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13728 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
