...
date: 1786563916 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +34.4%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.104× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.104× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8343
combined_client p50_us = 1407 · p99_us = 122879 · p999_us = 507903 · max_us = 3677516
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0xf6d5918e8920622c
...
tiering_cold_resolves = 160817760
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6673% (cold_reads 433886 · cold_resolves 867774 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 15871 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1279 µs (client tail spread 16160 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16160 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9483
combined_client p50_us = 1375 · p99_us = 102399 · p999_us = 442367 · max_us = 3165890
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x2e85388aa8aed510
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 162405250
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6716% (cold_reads 493405 · cold_resolves 986812 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 11775 · p999_us = 14079
  separation: mem_hit p99.9 14079 µs vs server cold p50 1279 µs (client tail spread 12864 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14079 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12864 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
