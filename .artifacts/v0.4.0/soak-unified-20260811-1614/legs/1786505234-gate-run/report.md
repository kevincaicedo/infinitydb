...
date: 1786505234 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +29.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.25 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.253× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.249× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8576
combined_client p50_us = 1439 · p99_us = 122879 · p999_us = 516095 · max_us = 4017297
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x266407fba6421da1
...
tiering_cold_resolves = 59929579
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.9026% (cold_reads 458194 · cold_resolves 925682 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1279 · p99_us = 11519 · p999_us = 13567
  separation: mem_hit p99.9 13567 µs vs server cold p50 1247 µs (client tail spread 12288 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13567 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12288 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 10215
combined_client p50_us = 1215 · p99_us = 94207 · p999_us = 450559 · max_us = 3640293
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x1c083b6e48e35667
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 61593372
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7928% (cold_reads 539000 · cold_resolves 1078119 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 9215 · p999_us = 11263
  separation: mem_hit p99.9 11263 µs vs server cold p50 1247 µs (client tail spread 10208 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11263 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10208 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
