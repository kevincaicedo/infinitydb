...
date: 1786482662 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +58.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 25.60 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.702× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.638× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
tiering_cold_resolves = 12198007
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 12.1315% (cold_reads 1015695 · cold_resolves 2240593 — re-resolve ratio 2.21×)
  mem_hit p50_us = 1631 · p99_us = 4607 · p999_us = 4735
  separation: mem_hit p99.9 4735 µs vs server cold p50 1183 µs (client tail spread 3104 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4735 µs >= server cold p50 1183 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 3104 µs vs cold service 1183 µs)
...
nils = 0
ops_per_sec = 18044
combined_client p50_us = 1471 · p99_us = 40959 · p999_us = 270335 · max_us = 2464642
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x1b5712da0005bfa6
tiering_cold_p50_us (worst cell) = 1183
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 16474311
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 11.2325% (cold_reads 1216050 · cold_resolves 2440795 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1279 · p99_us = 3647 · p999_us = 3839
  separation: mem_hit p99.9 3839 µs vs server cold p50 1183 µs (client tail spread 2560 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 3839 µs >= server cold p50 1183 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2560 µs vs cold service 1183 µs)
