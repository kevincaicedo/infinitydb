...
date: 1786583482 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +13.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.09 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 67.58 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.089× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.089× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9561
combined_client p50_us = 1215 · p99_us = 106495 · p999_us = 466943 · max_us = 3478212
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0xb25e4079acce3dd4
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
...
tiering_cold_resolves = 193742409
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7128% (cold_reads 499898 · cold_resolves 1002399 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1055 · p99_us = 11263 · p999_us = 13311
  separation: mem_hit p99.9 13311 µs vs server cold p50 1279 µs (client tail spread 12256 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13311 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12256 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 10525
combined_client p50_us = 1279 · p99_us = 83967 · p999_us = 376831 · max_us = 2595603
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x3a7e41079e8e8b95
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 195405565
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6670% (cold_reads 547334 · cold_resolves 1094672 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1119 · p99_us = 11519 · p999_us = 13567
  separation: mem_hit p99.9 13567 µs vs server cold p50 1279 µs (client tail spread 12448 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13567 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12448 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
