...
date: 1786500720 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s -0.4%)
...
|---|---|
| ycsb-a-zipfian | 1.289× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.284× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10186
combined_client p50_us = 1215 · p99_us = 102399 · p999_us = 483327 · max_us = 3944305
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x337846a1acc8c29b
...
tiering_cold_resolves = 52013439
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.0496% (cold_reads 553222 · cold_resolves 1125543 — re-resolve ratio 2.03×)
...
nils = 0
ops_per_sec = 9182
combined_client p50_us = 1439 · p99_us = 102399 · p999_us = 450559 · max_us = 3817300
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x4a7e7e8499332a65
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 59391
tiering_cold_p999_us (worst cell) = 327679
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 53519048
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7578% (cold_reads 482540 · cold_resolves 965100 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 13567 · p999_us = 16127
  separation: mem_hit p99.9 16127 µs vs server cold p50 1247 µs (client tail spread 14848 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16127 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14848 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
