...
date: 1786502223 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s +2.8%)
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.27 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.275× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.271× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10360
combined_client p50_us = 1215 · p99_us = 90111 · p999_us = 450559 · max_us = 3197018
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xdc02c32db811c0d7
...
tiering_cold_resolves = 54617225
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8007% (cold_reads 547027 · cold_resolves 1098177 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1055 · p99_us = 9727 · p999_us = 11519
  separation: mem_hit p99.9 11519 µs vs server cold p50 1247 µs (client tail spread 10464 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11519 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10464 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 10652
combined_client p50_us = 1215 · p99_us = 88063 · p999_us = 442367 · max_us = 3549479
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xde1d3ad2fd7881f1
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 56317686
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8011% (cold_reads 562551 · cold_resolves 1125360 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 8703 · p999_us = 10751
  separation: mem_hit p99.9 10751 µs vs server cold p50 1247 µs (client tail spread 9696 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10751 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9696 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
