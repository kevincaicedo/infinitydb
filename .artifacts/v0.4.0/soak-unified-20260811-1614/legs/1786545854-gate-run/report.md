...
date: 1786545854 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s -0.7%)
...
|---|---|
| ycsb-a-zipfian | 1.125× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.124× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8830
combined_client p50_us = 1599 · p99_us = 108543 · p999_us = 466943 · max_us = 4343463
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x1007c0001d6f4c6f
...
tiering_cold_resolves = 131196447
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7057% (cold_reads 461306 · cold_resolves 924301 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1407 · p99_us = 12031 · p999_us = 14335
  separation: mem_hit p99.9 14335 µs vs server cold p50 1279 µs (client tail spread 12928 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14335 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12928 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9448
combined_client p50_us = 1375 · p99_us = 100351 · p999_us = 425983 · max_us = 2514556
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xfe37f2924d3bb63f
tiering_cold_p50_us (worst cell) = 1279
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 132633835
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6658% (cold_reads 491227 · cold_resolves 982456 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 13311 · p999_us = 15871
  separation: mem_hit p99.9 15871 µs vs server cold p50 1279 µs (client tail spread 14656 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15871 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14656 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
