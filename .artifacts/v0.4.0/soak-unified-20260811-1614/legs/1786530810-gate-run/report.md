...
date: 1786530810 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +5.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.152× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.150× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8925
combined_client p50_us = 1375 · p99_us = 114687 · p999_us = 507903 · max_us = 3803902
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x33501ea339e97512
...
tiering_cold_resolves = 105208381
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6894% (cold_reads 465304 · cold_resolves 930611 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 10239 · p999_us = 12287
  separation: mem_hit p99.9 12287 µs vs server cold p50 1247 µs (client tail spread 11072 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12287 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11072 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 10400
combined_client p50_us = 1279 · p99_us = 88063 · p999_us = 417791 · max_us = 2403971
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x0f72b37f3329ab95
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 106809587
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8578% (cold_reads 552823 · cold_resolves 1105851 — re-resolve ratio 2.00×)
