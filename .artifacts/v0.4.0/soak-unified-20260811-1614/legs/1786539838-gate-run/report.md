...
date: 1786539838 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +43.8%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.134× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.133× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8644
combined_client p50_us = 1215 · p99_us = 114687 · p999_us = 499711 · max_us = 3841908
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x4444f74b4dcef323
...
tiering_cold_resolves = 120548145
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6964% (cold_reads 451330 · cold_resolves 902728 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 14591 · p999_us = 16895
...
nils = 0
ops_per_sec = 11499
combined_client p50_us = 1215 · p99_us = 71679 · p999_us = 401407 · max_us = 2500297
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x924f5cad510aa7f1
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 122404078
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6831% (cold_reads 599097 · cold_resolves 1198198 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 8447 · p999_us = 10495
  separation: mem_hit p99.9 10495 µs vs server cold p50 1279 µs (client tail spread 9440 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10495 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9440 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
