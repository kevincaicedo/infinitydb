...
date: 1786547358 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +19.8%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.123× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.122× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9203
combined_client p50_us = 1407 · p99_us = 104447 · p999_us = 475135 · max_us = 3906412
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x4afa50905bac7cd9
...
tiering_cold_resolves = 133593969
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6936% (cold_reads 480064 · cold_resolves 960134 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 11519 · p999_us = 13311
  separation: mem_hit p99.9 13311 µs vs server cold p50 1279 µs (client tail spread 12064 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13311 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12064 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 7548
combined_client p50_us = 1567 · p99_us = 135167 · p999_us = 516095 · max_us = 3398812
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xcf69dfb17f1b0f1f
tiering_cold_p50_us (worst cell) = 1279
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 134959587
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6871% (cold_reads 393701 · cold_resolves 787404 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1375 · p99_us = 17407 · p999_us = 18431
  separation: mem_hit p99.9 18431 µs vs server cold p50 1279 µs (client tail spread 17056 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 18431 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 17056 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
