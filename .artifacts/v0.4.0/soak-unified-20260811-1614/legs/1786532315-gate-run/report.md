...
date: 1786532315 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +33.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.149× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.147× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8720
combined_client p50_us = 1247 · p99_us = 114687 · p999_us = 516095 · max_us = 2817043
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x1c8a24c95ebb1d42
...
tiering_cold_resolves = 107718348
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6828% (cold_reads 454378 · cold_resolves 908761 — re-resolve ratio 2.00×)
...
nils = 0
ops_per_sec = 11189
combined_client p50_us = 1183 · p99_us = 79871 · p999_us = 409599 · max_us = 2634302
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x73f87413f820db18
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 109509441
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7346% (cold_reads 586463 · cold_resolves 1172930 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 8959 · p999_us = 10751
  separation: mem_hit p99.9 10751 µs vs server cold p50 1279 µs (client tail spread 9728 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10751 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9728 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
