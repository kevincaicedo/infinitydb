...
date: 1786518772 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
|---|---|
| ycsb-a-zipfian | 1.185× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.183× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10779
combined_client p50_us = 1215 · p99_us = 86015 · p999_us = 458751 · max_us = 3686005
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xd9315de88046c283
...
tiering_cold_resolves = 84114375
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8537% (cold_reads 572612 · cold_resolves 1155035 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1055 · p99_us = 7423 · p999_us = 9471
  separation: mem_hit p99.9 9471 µs vs server cold p50 1247 µs (client tail spread 8416 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9471 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8416 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 10739
combined_client p50_us = 1183 · p99_us = 81919 · p999_us = 393215 · max_us = 2732068
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x666eea4ea290feff
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 85795112
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8686% (cold_reads 571468 · cold_resolves 1143643 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 11263 · p999_us = 13567
  separation: mem_hit p99.9 13567 µs vs server cold p50 1247 µs (client tail spread 12512 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13567 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12512 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
