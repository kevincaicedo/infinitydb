...
date: 1786557889 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s +3.5%)
...
|---|---|
| ycsb-a-zipfian | 1.110× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.110× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8509
combined_client p50_us = 1375 · p99_us = 112639 · p999_us = 499711 · max_us = 4009163
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x5719c71eb629aab9
...
tiering_cold_resolves = 150975035
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6614% (cold_reads 442195 · cold_resolves 884392 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 14335 · p999_us = 16383
  separation: mem_hit p99.9 16383 µs vs server cold p50 1279 µs (client tail spread 15168 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16383 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15168 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 8814
combined_client p50_us = 1375 · p99_us = 110591 · p999_us = 475135 · max_us = 3498806
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x34292c43d6fd50a7
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 152359091
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7364% (cold_reads 462073 · cold_resolves 924148 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1183 · p99_us = 14079 · p999_us = 16383
  separation: mem_hit p99.9 16383 µs vs server cold p50 1279 µs (client tail spread 15200 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16383 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15200 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
