...
date: 1786533821 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +10.1%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.15 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.146× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.144× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8541
combined_client p50_us = 1375 · p99_us = 116735 · p999_us = 499711 · max_us = 4283009
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x95fe494069015ca6
...
tiering_cold_resolves = 110402364
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6892% (cold_reads 446458 · cold_resolves 892923 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 13055 · p999_us = 15615
  separation: mem_hit p99.9 15615 µs vs server cold p50 1279 µs (client tail spread 14400 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15615 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14400 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 8101
combined_client p50_us = 1407 · p99_us = 124927 · p999_us = 483327 · max_us = 2842258
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x0e0c9d35508f4f9f
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 111738143
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7143% (cold_reads 423562 · cold_resolves 847126 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 15871 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1279 µs (client tail spread 16128 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16128 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
