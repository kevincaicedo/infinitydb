...
date: 1786494696 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +6.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.35 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 55.30 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.355× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.347× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8809
combined_client p50_us = 1503 · p99_us = 110591 · p999_us = 483327 · max_us = 4796267
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x9da497e4b7e13903
tiering_cold_p50_us (worst cell) = 1215
tiering_cold_p99_us (worst cell) = 53247
tiering_cold_p999_us (worst cell) = 311295
...
tiering_cold_resolves = 41455192
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.2221% (cold_reads 487447 · cold_resolves 997351 — re-resolve ratio 2.05×)
  mem_hit p50_us = 1311 · p99_us = 10751 · p999_us = 12543
  separation: mem_hit p99.9 12543 µs vs server cold p50 1215 µs (client tail spread 11232 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12543 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11232 µs vs cold service 1215 µs)
...
nils = 0
ops_per_sec = 10029
combined_client p50_us = 1183 · p99_us = 96255 · p999_us = 425983 · max_us = 2352989
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xcdc6b46950ef5bd0
tiering_cold_p50_us (worst cell) = 1215
tiering_cold_p99_us (worst cell) = 55295
tiering_cold_p999_us (worst cell) = 319487
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 43119263
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.1345% (cold_reads 549702 · cold_resolves 1104750 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1055 · p99_us = 10495 · p999_us = 12287
  separation: mem_hit p99.9 12287 µs vs server cold p50 1215 µs (client tail spread 11232 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12287 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11232 µs vs cold service 1215 µs)
