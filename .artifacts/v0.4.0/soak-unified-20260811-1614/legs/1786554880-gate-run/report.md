...
date: 1786554880 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
|---|---|
| ycsb-a-zipfian | 1.113× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.113× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10850
combined_client p50_us = 1183 · p99_us = 86015 · p999_us = 417791 · max_us = 2845462
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x13d9aded0fe7e10e
...
tiering_cold_resolves = 145923137
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6607% (cold_reads 563814 · cold_resolves 1127633 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 9983 · p999_us = 11519
  separation: mem_hit p99.9 11519 µs vs server cold p50 1279 µs (client tail spread 10496 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11519 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10496 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9247
combined_client p50_us = 1439 · p99_us = 104447 · p999_us = 499711 · max_us = 3411104
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0xc5c1a78fa82d34b8
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 147529865
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6664% (cold_reads 480839 · cold_resolves 961681 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 11007 · p999_us = 13055
  separation: mem_hit p99.9 13055 µs vs server cold p50 1279 µs (client tail spread 11776 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13055 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11776 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
