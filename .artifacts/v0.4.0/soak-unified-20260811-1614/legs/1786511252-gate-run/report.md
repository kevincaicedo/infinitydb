...
date: 1786511252 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.22 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.219× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.216× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10404
combined_client p50_us = 1247 · p99_us = 88063 · p999_us = 425983 · max_us = 3211307
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x0b4982d8eca44d3f
...
tiering_cold_resolves = 70438158
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8439% (cold_reads 552069 · cold_resolves 1108354 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1087 · p99_us = 9983 · p999_us = 11775
  separation: mem_hit p99.9 11775 µs vs server cold p50 1247 µs (client tail spread 10688 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11775 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10688 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 8023
combined_client p50_us = 1535 · p99_us = 124927 · p999_us = 507903 · max_us = 3193353
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x168452903d3e2b49
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 62463
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 71885843
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.0429% (cold_reads 435303 · cold_resolves 870610 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1375 · p99_us = 15871 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1247 µs (client tail spread 16032 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16032 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
