...
date: 1786526297 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +38.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.163× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.161× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9187
combined_client p50_us = 1439 · p99_us = 92159 · p999_us = 434175 · max_us = 3874867
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x6f0a3fd5b54f64df
...
tiering_cold_resolves = 97498672
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7248% (cold_reads 480922 · cold_resolves 963494 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 14335 · p999_us = 16383
  separation: mem_hit p99.9 16383 µs vs server cold p50 1247 µs (client tail spread 15104 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16383 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15104 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 8806
combined_client p50_us = 1471 · p99_us = 110591 · p999_us = 483327 · max_us = 2979284
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x98a273acb9a05e4f
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 99145224
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7520% (cold_reads 462470 · cold_resolves 924953 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1311 · p99_us = 13055 · p999_us = 15871
  separation: mem_hit p99.9 15871 µs vs server cold p50 1247 µs (client tail spread 14560 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15871 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14560 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
