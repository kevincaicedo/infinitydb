...
date: 1786577458 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s -3.2%)
...
|---|---|
| ycsb-a-zipfian | 1.093× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.093× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9038
combined_client p50_us = 1247 · p99_us = 108543 · p999_us = 491519 · max_us = 4089876
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x63c19dd90d49d937
...
tiering_cold_resolves = 183725223
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7638% (cold_reads 475277 · cold_resolves 951745 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 13567 · p999_us = 15871
  separation: mem_hit p99.9 15871 µs vs server cold p50 1279 µs (client tail spread 14784 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15871 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14784 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 11367
combined_client p50_us = 1183 · p99_us = 75775 · p999_us = 376831 · max_us = 2451194
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x650eafc6df0fc86d
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 185363528
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6648% (cold_reads 592394 · cold_resolves 1184846 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1007 · p99_us = 9727 · p999_us = 11775
  separation: mem_hit p99.9 11775 µs vs server cold p50 1279 µs (client tail spread 10768 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11775 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10768 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
