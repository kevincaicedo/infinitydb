...
date: 1786508242 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +8.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.235× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.232× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8541
combined_client p50_us = 1343 · p99_us = 114687 · p999_us = 540671 · max_us = 3032545
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0xc339fbfa4ffed860
...
tiering_cold_resolves = 65216148
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7461% (cold_reads 448224 · cold_resolves 897645 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 13567 · p999_us = 16127
  separation: mem_hit p99.9 16127 µs vs server cold p50 1247 µs (client tail spread 14912 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16127 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14912 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 10497
combined_client p50_us = 1279 · p99_us = 88063 · p999_us = 409599 · max_us = 2926386
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xb6c15a57a9c80bcc
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 61439
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 66847741
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.0845% (cold_reads 572149 · cold_resolves 1146590 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1119 · p99_us = 8703 · p999_us = 10495
  separation: mem_hit p99.9 10495 µs vs server cold p50 1247 µs (client tail spread 9376 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10495 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9376 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
