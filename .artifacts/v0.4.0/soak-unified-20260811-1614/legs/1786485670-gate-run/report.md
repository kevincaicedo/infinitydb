...
date: 1786485670 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +38.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 38.91 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.513× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.497× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 6813
combined_client p50_us = 2495 · p99_us = 147455 · p999_us = 589823 · max_us = 4394624
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x355c66ffd07357c5
tiering_cold_p50_us (worst cell) = 1183
tiering_cold_p99_us (worst cell) = 30207
tiering_cold_p999_us (worst cell) = 233471
...
tiering_cold_resolves = 25050463
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 21.9550% (cold_reads 897582 · cold_resolves 2297112 — re-resolve ratio 2.56×)
  mem_hit p50_us = 1983 · p99_us = 4351 · p999_us = 4479
  separation: mem_hit p99.9 4479 µs vs server cold p50 1183 µs (client tail spread 2496 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 4479 µs >= server cold p50 1183 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 2496 µs vs cold service 1183 µs)
...
nils = 0
ops_per_sec = 9263
combined_client p50_us = 1663 · p99_us = 96255 · p999_us = 466943 · max_us = 4072277
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xd87617ae01bde6d7
tiering_cold_p50_us (worst cell) = 1183
tiering_cold_p99_us (worst cell) = 38911
tiering_cold_p999_us (worst cell) = 245759
cold_read_qd_p99 (worst cell) = 5
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 26919599
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 11.4273% (cold_reads 635195 · cold_resolves 1283751 — re-resolve ratio 2.02×)
  mem_hit p50_us = 1439 · p99_us = 6271 · p999_us = 7039
  separation: mem_hit p99.9 7039 µs vs server cold p50 1183 µs (client tail spread 5600 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 7039 µs >= server cold p50 1183 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 5600 µs vs cold service 1183 µs)
