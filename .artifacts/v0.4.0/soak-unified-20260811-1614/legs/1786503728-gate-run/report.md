...
date: 1786503728 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +14.1%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.26 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.263× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.259× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9768
combined_client p50_us = 1183 · p99_us = 104447 · p999_us = 483327 · max_us = 4158472
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x10dcbe0b04d4000e
...
tiering_cold_resolves = 57344751
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7489% (cold_reads 512849 · cold_resolves 1027065 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 9215 · p999_us = 11007
  separation: mem_hit p99.9 11007 µs vs server cold p50 1247 µs (client tail spread 9984 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11007 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9984 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9991
combined_client p50_us = 1311 · p99_us = 92159 · p999_us = 425983 · max_us = 4139030
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x9bc078e945f2309e
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 60415
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 59003897
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.9478% (cold_reads 536394 · cold_resolves 1073652 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1119 · p99_us = 10751 · p999_us = 12799
  separation: mem_hit p99.9 12799 µs vs server cold p50 1247 µs (client tail spread 11680 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12799 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11680 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
