...
date: 1786499216 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +48.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.30 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.304× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.298× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 7992
combined_client p50_us = 1311 · p99_us = 135167 · p999_us = 557055 · max_us = 3363421
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xcc13226bf429f442
...
tiering_cold_resolves = 49364365
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8207% (cold_reads 423106 · cold_resolves 849929 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1119 · p99_us = 13567 · p999_us = 16127
  separation: mem_hit p99.9 16127 µs vs server cold p50 1247 µs (client tail spread 15008 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16127 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15008 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 8175
combined_client p50_us = 1407 · p99_us = 122879 · p999_us = 475135 · max_us = 2622614
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xe478a9f972f2b407
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 50887896
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7791% (cold_reads 430602 · cold_resolves 861241 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 16127 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1247 µs (client tail spread 16160 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16160 µs vs cold service 1247 µs)
