...
date: 1786572942 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s -4.1%)
...
|---|---|
| ycsb-a-zipfian | 1.097× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.096× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8199
combined_client p50_us = 1247 · p99_us = 126975 · p999_us = 540671 · max_us = 3841122
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xa0e9f7235db9c815
...
tiering_cold_resolves = 176067580
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6688% (cold_reads 426712 · cold_resolves 853430 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 14079 · p999_us = 16383
...
nils = 0
ops_per_sec = 10379
combined_client p50_us = 1279 · p99_us = 83967 · p999_us = 417791 · max_us = 3428056
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xedc6b82e975c5363
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 177556900
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6797% (cold_reads 540525 · cold_resolves 1081056 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 11263 · p999_us = 13567
  separation: mem_hit p99.9 13567 µs vs server cold p50 1279 µs (client tail spread 12480 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13567 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12480 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
