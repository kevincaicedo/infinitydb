...
date: 1786553377 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +8.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.115× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.115× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9930
combined_client p50_us = 1215 · p99_us = 96255 · p999_us = 442367 · max_us = 3639417
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xb6664063a4523a6b
...
tiering_cold_resolves = 143114035
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6629% (cold_reads 516148 · cold_resolves 1032303 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 11263 · p999_us = 13055
  separation: mem_hit p99.9 13055 µs vs server cold p50 1279 µs (client tail spread 12000 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13055 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12000 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 10715
combined_client p50_us = 1279 · p99_us = 88063 · p999_us = 401407 · max_us = 2772482
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x83d0a902cf414c17
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 144795504
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6617% (cold_reads 556887 · cold_resolves 1113777 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1151 · p99_us = 9471 · p999_us = 11263
  separation: mem_hit p99.9 11263 µs vs server cold p50 1279 µs (client tail spread 10112 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11263 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10112 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
