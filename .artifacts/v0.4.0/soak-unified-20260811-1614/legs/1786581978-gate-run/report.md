...
date: 1786581978 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +82.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.090× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.090× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 7341
combined_client p50_us = 1567 · p99_us = 139263 · p999_us = 524287 · max_us = 3182100
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x18a8872176002582
...
tiering_cold_resolves = 191205956
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6689% (cold_reads 382795 · cold_resolves 765592 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1407 · p99_us = 17407 · p999_us = 18431
  separation: mem_hit p99.9 18431 µs vs server cold p50 1279 µs (client tail spread 17024 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 18431 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 17024 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 7786
combined_client p50_us = 1439 · p99_us = 124927 · p999_us = 516095 · max_us = 4893048
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xc9ff5ff60040e911
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 192740010
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7402% (cold_reads 408359 · cold_resolves 816726 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 16895 · p999_us = 17919
  separation: mem_hit p99.9 17919 µs vs server cold p50 1279 µs (client tail spread 16640 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17919 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16640 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
