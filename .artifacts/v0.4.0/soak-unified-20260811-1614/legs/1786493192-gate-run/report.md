...
date: 1786493192 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +21.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.38 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.375× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.367× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8679
combined_client p50_us = 1247 · p99_us = 122879 · p999_us = 491519 · max_us = 5861226
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x6e77e0b1b12bcd60
tiering_cold_p50_us (worst cell) = 1215
tiering_cold_p99_us (worst cell) = 48127
...
tiering_cold_resolves = 38773520
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.9833% (cold_reads 467875 · cold_resolves 940264 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1087 · p99_us = 12031 · p999_us = 14591
  separation: mem_hit p99.9 14591 µs vs server cold p50 1215 µs (client tail spread 13504 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14591 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13504 µs vs cold service 1215 µs)
...
nils = 0
ops_per_sec = 9365
combined_client p50_us = 1343 · p99_us = 102399 · p999_us = 450559 · max_us = 3529746
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xab34ba0ffde5afc8
tiering_cold_p50_us (worst cell) = 1215
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 40457841
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.1577% (cold_reads 514612 · cold_resolves 1030584 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1151 · p99_us = 12031 · p999_us = 14591
  separation: mem_hit p99.9 14591 µs vs server cold p50 1215 µs (client tail spread 13440 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14591 µs >= server cold p50 1215 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13440 µs vs cold service 1215 µs)
tripwire sqes/submit = 6.0
```
