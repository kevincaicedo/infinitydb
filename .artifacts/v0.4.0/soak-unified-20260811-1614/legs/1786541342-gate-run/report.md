...
date: 1786541342 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +14.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.131× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.130× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10477
combined_client p50_us = 1183 · p99_us = 92159 · p999_us = 450559 · max_us = 4589183
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x1f4b8ebf2f68b2fc
...
tiering_cold_resolves = 123503563
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7434% (cold_reads 549645 · cold_resolves 1099485 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 7935 · p999_us = 10239
...
nils = 0
ops_per_sec = 10001
combined_client p50_us = 1439 · p99_us = 92159 · p999_us = 417791 · max_us = 2393162
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xb502da0b27dbdf3c
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 125206988
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.9698% (cold_reads 538258 · cold_resolves 1077297 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 9983 · p999_us = 11519
  separation: mem_hit p99.9 11519 µs vs server cold p50 1279 µs (client tail spread 10240 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11519 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10240 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
