...
date: 1786556385 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +10.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.112× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.111× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8907
combined_client p50_us = 1215 · p99_us = 122879 · p999_us = 507903 · max_us = 3313615
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xe5ac22ca5b37ffab
...
tiering_cold_resolves = 148455117
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6555% (cold_reads 462594 · cold_resolves 925252 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 11519 · p999_us = 13823
  separation: mem_hit p99.9 13823 µs vs server cold p50 1279 µs (client tail spread 12768 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 13823 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12768 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 10774
combined_client p50_us = 1311 · p99_us = 86015 · p999_us = 376831 · max_us = 2442404
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x760cb35961897382
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 150090643
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6723% (cold_reads 560620 · cold_resolves 1121242 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1183 · p99_us = 10239 · p999_us = 12031
  separation: mem_hit p99.9 12031 µs vs server cold p50 1279 µs (client tail spread 10848 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12031 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10848 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
