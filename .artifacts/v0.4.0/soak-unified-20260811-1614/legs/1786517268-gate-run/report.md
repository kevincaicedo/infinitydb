...
date: 1786517268 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +39.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.19 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.192× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.189× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9330
combined_client p50_us = 1183 · p99_us = 110591 · p999_us = 499711 · max_us = 3228278
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xad98ce028cdd2627
...
tiering_cold_resolves = 80954867
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7789% (cold_reads 491476 · cold_resolves 983266 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 11007 · p999_us = 12799
  separation: mem_hit p99.9 12799 µs vs server cold p50 1247 µs (client tail spread 11776 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12799 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11776 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 12392
combined_client p50_us = 1215 · p99_us = 67583 · p999_us = 344063 · max_us = 2232609
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xc3ab64d7ddd28467
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 82959340
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8474% (cold_reads 657857 · cold_resolves 1315832 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 7295 · p999_us = 9215
  separation: mem_hit p99.9 9215 µs vs server cold p50 1247 µs (client tail spread 8160 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9215 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8160 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
