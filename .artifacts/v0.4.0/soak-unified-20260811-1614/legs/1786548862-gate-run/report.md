...
date: 1786548862 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s -14.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.121× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.120× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9440
combined_client p50_us = 1279 · p99_us = 102399 · p999_us = 483327 · max_us = 4035301
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xf48534d4acca6956
...
tiering_cold_resolves = 135945039
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6942% (cold_reads 492498 · cold_resolves 985452 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 11775 · p999_us = 14079
  separation: mem_hit p99.9 14079 µs vs server cold p50 1279 µs (client tail spread 12992 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14079 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 12992 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 8578
combined_client p50_us = 1471 · p99_us = 108543 · p999_us = 483327 · max_us = 3082438
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x067ee40a385b93c2
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 137260414
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6896% (cold_reads 447367 · cold_resolves 894737 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1311 · p99_us = 14591 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1279 µs (client tail spread 15584 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15584 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
