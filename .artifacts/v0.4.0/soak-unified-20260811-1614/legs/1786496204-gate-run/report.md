...
date: 1786496204 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +13.4%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.34 | PASS |
...
| Cold reads: p99 < 1.5 ms on NVMe under loaded zipfian rows | < 1.5 ms, cold-read split histogram, worst loaded row | 57.34 | FAIL (DEV-TIER, non-binding) |
...
|---|---|
| ycsb-a-zipfian | 1.336× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.328× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9640
combined_client p50_us = 1343 · p99_us = 102399 · p999_us = 483327 · max_us = 4707552
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x32b1272d23bd69cd
...
tiering_cold_resolves = 44201095
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.1608% (cold_reads 530012 · cold_resolves 1081832 — re-resolve ratio 2.04×)
  mem_hit p50_us = 1151 · p99_us = 8959 · p999_us = 10751
  separation: mem_hit p99.9 10751 µs vs server cold p50 1247 µs (client tail spread 9600 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10751 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9600 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9447
combined_client p50_us = 1311 · p99_us = 104447 · p999_us = 458751 · max_us = 3965170
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0xb6af9965cda0b105
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 45855496
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 9.0855% (cold_reads 515013 · cold_resolves 1033714 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1119 · p99_us = 9983 · p999_us = 12031
  separation: mem_hit p99.9 12031 µs vs server cold p50 1247 µs (client tail spread 10912 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12031 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10912 µs vs cold service 1247 µs)
