...
date: 1786514261 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +47.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.204× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.201× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9447
combined_client p50_us = 1215 · p99_us = 106495 · p999_us = 483327 · max_us = 3086192
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x8eabbf0726f0db48
...
tiering_cold_resolves = 75619164
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7646% (cold_reads 497299 · cold_resolves 995037 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 10751 · p999_us = 12543
  separation: mem_hit p99.9 12543 µs vs server cold p50 1247 µs (client tail spread 11488 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12543 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11488 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 11295
combined_client p50_us = 1247 · p99_us = 79871 · p999_us = 376831 · max_us = 2503963
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x51f3044ad7d04e87
tiering_cold_p50_us (worst cell) = 1247
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 77581727
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8098% (cold_reads 597031 · cold_resolves 1196288 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 9215 · p999_us = 11007
  separation: mem_hit p99.9 11007 µs vs server cold p50 1247 µs (client tail spread 9952 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11007 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9952 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
