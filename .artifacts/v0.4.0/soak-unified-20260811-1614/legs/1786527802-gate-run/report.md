...
date: 1786527802 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +36.3%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.16 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.159× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.157× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8920
combined_client p50_us = 1343 · p99_us = 112639 · p999_us = 458751 · max_us = 4358339
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xa12dd438702a18fb
...
tiering_cold_resolves = 100076674
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6824% (cold_reads 465702 · cold_resolves 931450 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1215 · p99_us = 12543 · p999_us = 15103
  separation: mem_hit p99.9 15103 µs vs server cold p50 1247 µs (client tail spread 13888 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15103 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13888 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9483
combined_client p50_us = 1471 · p99_us = 98303 · p999_us = 409599 · max_us = 2806055
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xd89db6527c10093c
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 101707625
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7604% (cold_reads 498464 · cold_resolves 997613 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 13055 · p999_us = 15615
  separation: mem_hit p99.9 15615 µs vs server cold p50 1247 µs (client tail spread 14336 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15615 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14336 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
