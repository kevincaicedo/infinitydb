...
date: 1786535326 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +17.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.143× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.141× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10285
combined_client p50_us = 1215 · p99_us = 94207 · p999_us = 458751 · max_us = 3421822
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x99ff699ec8069ff2
...
tiering_cold_resolves = 112814930
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7147% (cold_reads 538019 · cold_resolves 1076787 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 9215 · p999_us = 11007
  separation: mem_hit p99.9 11007 µs vs server cold p50 1279 µs (client tail spread 9952 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 11007 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9952 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 8691
combined_client p50_us = 1503 · p99_us = 106495 · p999_us = 458751 · max_us = 2882527
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x8f2e2ae643bcde99
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 114380277
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6878% (cold_reads 453528 · cold_resolves 907064 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1343 · p99_us = 15359 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1279 µs (client tail spread 15552 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15552 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
