...
date: 1786578965 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +29.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.092× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.092× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9270
combined_client p50_us = 1279 · p99_us = 100351 · p999_us = 466943 · max_us = 3310066
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x67a67140f1ff6b5d
...
tiering_cold_resolves = 186326890
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6599% (cold_reads 481678 · cold_resolves 963362 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1119 · p99_us = 14079 · p999_us = 16383
  separation: mem_hit p99.9 16383 µs vs server cold p50 1279 µs (client tail spread 15264 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16383 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15264 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 7969
combined_client p50_us = 1567 · p99_us = 124927 · p999_us = 524287 · max_us = 3539319
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xf365a8c3ca7b8bfd
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 187790839
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7565% (cold_reads 418938 · cold_resolves 837880 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1375 · p99_us = 14591 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1279 µs (client tail spread 15520 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15520 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
