...
date: 1786586492 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +41.5%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.087× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.087× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8431
combined_client p50_us = 1439 · p99_us = 122879 · p999_us = 507903 · max_us = 3697668
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x0c992c2a2a4528c0
...
tiering_cold_resolves = 198717453
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7440% (cold_reads 442984 · cold_resolves 886014 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 12799 · p999_us = 15359
  separation: mem_hit p99.9 15359 µs vs server cold p50 1279 µs (client tail spread 14080 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15359 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14080 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9677
combined_client p50_us = 1407 · p99_us = 100351 · p999_us = 442367 · max_us = 2950159
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x8bfab953c75d23e1
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 200354398
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6695% (cold_reads 503815 · cold_resolves 1007649 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 12031 · p999_us = 14335
  separation: mem_hit p99.9 14335 µs vs server cold p50 1279 µs (client tail spread 13088 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14335 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13088 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
