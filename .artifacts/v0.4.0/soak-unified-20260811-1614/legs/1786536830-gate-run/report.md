...
date: 1786536830 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
|---|---|
| ycsb-a-zipfian | 1.139× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.138× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9952
combined_client p50_us = 1311 · p99_us = 102399 · p999_us = 466943 · max_us = 2937394
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xa26a9506f1bc7478
...
tiering_cold_resolves = 115422545
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7181% (cold_reads 520610 · cold_resolves 1042268 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1183 · p99_us = 8447 · p999_us = 10495
  separation: mem_hit p99.9 10495 µs vs server cold p50 1279 µs (client tail spread 9312 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10495 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9312 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9693
combined_client p50_us = 1439 · p99_us = 92159 · p999_us = 425983 · max_us = 2948942
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xf1107d90406bcc5d
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 64511
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 116943382
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6944% (cold_reads 505654 · cold_resolves 1011310 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 12287 · p999_us = 14847
  separation: mem_hit p99.9 14847 µs vs server cold p50 1279 µs (client tail spread 13568 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14847 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13568 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
