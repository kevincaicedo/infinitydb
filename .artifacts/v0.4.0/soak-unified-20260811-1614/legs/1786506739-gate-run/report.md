...
date: 1786506739 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +20.7%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| Write amplification, worst tiered namespace | < 3 x user bytes (wal + flush) | 1.24 | PASS |
...
|---|---|
| ycsb-a-zipfian | 1.243× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.240× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8728
combined_client p50_us = 1343 · p99_us = 110591 · p999_us = 483327 · max_us = 3851954
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xffbd9f546c93911b
...
tiering_cold_resolves = 62510340
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7413% (cold_reads 457802 · cold_resolves 916968 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1151 · p99_us = 14079 · p999_us = 16127
  separation: mem_hit p99.9 16127 µs vs server cold p50 1247 µs (client tail spread 14976 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16127 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14976 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 11753
combined_client p50_us = 1183 · p99_us = 73727 · p999_us = 360447 · max_us = 2912979
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.25%
stream_checksum = 0x1d2cbcdabdc4ea51
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 61439
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 64318503
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8375% (cold_reads 623356 · cold_resolves 1246926 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1007 · p99_us = 8703 · p999_us = 10751
  separation: mem_hit p99.9 10751 µs vs server cold p50 1247 µs (client tail spread 9744 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10751 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9744 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
