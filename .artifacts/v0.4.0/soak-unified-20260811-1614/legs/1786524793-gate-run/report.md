...
date: 1786524793 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
|---|---|
| ycsb-a-zipfian | 1.167× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.165× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9763
combined_client p50_us = 1503 · p99_us = 96255 · p999_us = 442367 · max_us = 3199775
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xd9236804a26a8fbb
...
tiering_cold_resolves = 94770934
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7489% (cold_reads 513074 · cold_resolves 1029148 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1343 · p99_us = 10751 · p999_us = 12287
  separation: mem_hit p99.9 12287 µs vs server cold p50 1247 µs (client tail spread 10944 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12287 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 10944 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 11633
combined_client p50_us = 1183 · p99_us = 71679 · p999_us = 376831 · max_us = 2591108
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x121dfa7d331d07af
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
tiering_cold_p999_us (worst cell) = 335871
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 96535178
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7953% (cold_reads 613938 · cold_resolves 1228439 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 7551 · p999_us = 9727
  separation: mem_hit p99.9 9727 µs vs server cold p50 1247 µs (client tail spread 8704 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9727 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8704 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
