...
date: 1786565420 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
|---|---|
| ycsb-a-zipfian | 1.103× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.102× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10239
combined_client p50_us = 1151 · p99_us = 98303 · p999_us = 475135 · max_us = 4638102
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xb02ff1bd2d472de3
...
tiering_cold_resolves = 163482099
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7310% (cold_reads 536364 · cold_resolves 1076849 — re-resolve ratio 2.01×)
  mem_hit p50_us = 1007 · p99_us = 8447 · p999_us = 10495
  separation: mem_hit p99.9 10495 µs vs server cold p50 1279 µs (client tail spread 9488 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10495 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9488 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 10355
combined_client p50_us = 1247 · p99_us = 88063 · p999_us = 434175 · max_us = 3062231
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x3e9f5c00c595ea35
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 165136184
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6577% (cold_reads 537908 · cold_resolves 1075818 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 10751 · p999_us = 12543
  separation: mem_hit p99.9 12543 µs vs server cold p50 1279 µs (client tail spread 11456 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12543 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11456 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
