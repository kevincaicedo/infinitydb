...
date: 1786559396 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +17.1%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.109× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.108× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9018
combined_client p50_us = 1471 · p99_us = 106495 · p999_us = 491519 · max_us = 3955917
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x03a7e27ae939bbe5
...
tiering_cold_resolves = 153297349
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6638% (cold_reads 469128 · cold_resolves 938258 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1311 · p99_us = 12799 · p999_us = 15615
  separation: mem_hit p99.9 15615 µs vs server cold p50 1279 µs (client tail spread 14304 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15615 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14304 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 10134
combined_client p50_us = 1247 · p99_us = 88063 · p999_us = 409599 · max_us = 2986007
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x773e4c4f83feaeaa
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 154902640
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6621% (cold_reads 528048 · cold_resolves 1056098 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 12543 · p999_us = 15103
  separation: mem_hit p99.9 15103 µs vs server cold p50 1279 µs (client tail spread 14016 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15103 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14016 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
