...
date: 1786587996 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +39.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
| ycsb-b-zipfian | 1.086× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 7483
combined_client p50_us = 1695 · p99_us = 135167 · p999_us = 516095 · max_us = 4000888
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x2781a53f11a4ea7e
...
tiering_cold_resolves = 201131730
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6540% (cold_reads 388665 · cold_resolves 777332 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1535 · p99_us = 16383 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1279 µs (client tail spread 15872 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15872 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9088
combined_client p50_us = 1407 · p99_us = 104447 · p999_us = 483327 · max_us = 2525280
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x050cdfd4479c874e
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 202638089
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8173% (cold_reads 480799 · cold_resolves 961602 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 12031 · p999_us = 14591
  separation: mem_hit p99.9 14591 µs vs server cold p50 1279 µs (client tail spread 13344 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14591 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13344 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
