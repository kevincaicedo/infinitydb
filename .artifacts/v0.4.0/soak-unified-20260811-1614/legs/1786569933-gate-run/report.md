...
date: 1786569933 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +20.2%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.099× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.099× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8161
combined_client p50_us = 1471 · p99_us = 124927 · p999_us = 499711 · max_us = 2977983
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xe28c05c26d1a7a03
...
tiering_cold_resolves = 171115839
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6635% (cold_reads 424255 · cold_resolves 848563 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1311 · p99_us = 15615 · p999_us = 17407
  separation: mem_hit p99.9 17407 µs vs server cold p50 1279 µs (client tail spread 16096 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 17407 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 16096 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9143
combined_client p50_us = 1247 · p99_us = 108543 · p999_us = 442367 · max_us = 2329491
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x9d0f2d13908227cd
tiering_cold_p50_us (worst cell) = 1279
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 172575986
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6679% (cold_reads 475495 · cold_resolves 950996 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 14335 · p999_us = 16383
  separation: mem_hit p99.9 16383 µs vs server cold p50 1279 µs (client tail spread 15328 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16383 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15328 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
