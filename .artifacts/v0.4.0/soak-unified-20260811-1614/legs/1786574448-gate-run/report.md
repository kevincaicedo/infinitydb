...
date: 1786574448 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s -3.7%)
...
|---|---|
| ycsb-a-zipfian | 1.096× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.095× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 10067
combined_client p50_us = 1183 · p99_us = 102399 · p999_us = 475135 · max_us = 4363451
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xcf05da5a05216c2d
...
tiering_cold_resolves = 178604300
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6701% (cold_reads 523696 · cold_resolves 1047400 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 8191 · p999_us = 10239
  separation: mem_hit p99.9 10239 µs vs server cold p50 1279 µs (client tail spread 9216 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 10239 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 9216 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 11413
combined_client p50_us = 1215 · p99_us = 81919 · p999_us = 401407 · max_us = 4150508
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x34b26d7b00ffe970
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 180310949
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7447% (cold_reads 600738 · cold_resolves 1202410 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 7807 · p999_us = 9983
  separation: mem_hit p99.9 9983 µs vs server cold p50 1279 µs (client tail spread 8928 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 9983 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 8928 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
