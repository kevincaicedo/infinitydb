...
date: 1786566924 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +19.6%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.102× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.101× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 8643
combined_client p50_us = 1375 · p99_us = 120831 · p999_us = 516095 · max_us = 3065921
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.22%
stream_checksum = 0x8d8452db89e4dc8b
...
tiering_cold_resolves = 166036868
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6840% (cold_reads 450341 · cold_resolves 900684 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1247 · p99_us = 13311 · p999_us = 15871
  separation: mem_hit p99.9 15871 µs vs server cold p50 1279 µs (client tail spread 14624 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15871 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14624 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9950
combined_client p50_us = 1279 · p99_us = 88063 · p999_us = 434175 · max_us = 2778658
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xc7b64280fbcbe929
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 65535
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 167615299
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6650% (cold_reads 517335 · cold_resolves 1034676 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 12031 · p999_us = 14847
  separation: mem_hit p99.9 14847 µs vs server cold p50 1279 µs (client tail spread 13760 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 14847 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 13760 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
