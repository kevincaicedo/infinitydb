...
date: 1786571438 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +18.9%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.098× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.097× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9852
combined_client p50_us = 1183 · p99_us = 100351 · p999_us = 483327 · max_us = 3264238
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xa9193f9db90b576c
...
tiering_cold_resolves = 173609683
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7415% (cold_reads 516809 · cold_resolves 1033697 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1055 · p99_us = 10495 · p999_us = 12287
  separation: mem_hit p99.9 12287 µs vs server cold p50 1279 µs (client tail spread 11232 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12287 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11232 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 9544
combined_client p50_us = 1279 · p99_us = 94207 · p999_us = 450559 · max_us = 2898495
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xf032d323482677d3
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 175214150
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6646% (cold_reads 496223 · cold_resolves 992452 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 13311 · p999_us = 15871
