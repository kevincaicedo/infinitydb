...
date: 1786575953 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): GENERATOR-LIMITED at 8 conns (+50% moved ops/s +13.0%) — absolutes understate the server; deltas remain valid at fixed generator config
...
|---|---|
| ycsb-a-zipfian | 1.094× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.094× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9352
combined_client p50_us = 1183 · p99_us = 112639 · p999_us = 491519 · max_us = 2876956
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0xac18f9d9bb8189ea
...
tiering_cold_resolves = 181287392
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6716% (cold_reads 487859 · cold_resolves 976443 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1023 · p99_us = 10239 · p999_us = 12031
  separation: mem_hit p99.9 12031 µs vs server cold p50 1279 µs (client tail spread 11008 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12031 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11008 µs vs cold service 1279 µs)
...
nils = 0
ops_per_sec = 8999
combined_client p50_us = 1343 · p99_us = 108543 · p999_us = 466943 · max_us = 2518039
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.23%
stream_checksum = 0x6a6d7593b6545699
tiering_cold_p50_us (worst cell) = 1279
tiering_cold_p99_us (worst cell) = 67583
tiering_cold_p999_us (worst cell) = 344063
cold_read_qd_p99 (worst cell) = 4
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 182773478
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.6629% (cold_reads 467727 · cold_resolves 935458 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1183 · p99_us = 15103 · p999_us = 16895
  separation: mem_hit p99.9 16895 µs vs server cold p50 1279 µs (client tail spread 15712 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 16895 µs >= server cold p50 1279 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 15712 µs vs cold service 1279 µs)
tripwire sqes/submit = 5.9
```
