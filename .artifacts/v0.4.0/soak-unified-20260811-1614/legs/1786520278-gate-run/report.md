...
date: 1786520278 (unix) · cells: 4 · conns: 8 · pipeline: 8 · duration: 600s · dataset: 10× budget · ycsb rows: 2 (M4-S22)
env-check: OK
...
- saturation (ycsb-a-zipfian): generator unsaturated at 8 conns (+50% conns moved ops/s +3.6%)
...
|---|---|
| ycsb-a-zipfian | 1.180× worst of 4 namespace(s) · blob: n/a (no blob activity) |
| ycsb-b-zipfian | 1.178× worst of 4 namespace(s) · blob: n/a (no blob activity) |
...
nils = 0
ops_per_sec = 9608
combined_client p50_us = 1439 · p99_us = 102399 · p999_us = 458751 · max_us = 2459393
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0xee40a3b319df3a29
...
tiering_cold_resolves = 86805609
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.7402% (cold_reads 504389 · cold_resolves 1010497 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1279 · p99_us = 10751 · p999_us = 12543
  separation: mem_hit p99.9 12543 µs vs server cold p50 1247 µs (client tail spread 11264 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 12543 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 11264 µs vs cold service 1247 µs)
...
nils = 0
ops_per_sec = 9801
combined_client p50_us = 1247 · p99_us = 92159 · p999_us = 425983 · max_us = 4498163
(combined = context only; the split section below is the honest read)
hot_share_top1pct = 72.24%
stream_checksum = 0x79a1d48b446f4d15
tiering_cold_p50_us (worst cell) = 1247
tiering_cold_p99_us (worst cell) = 63487
...
coalesce_ratio_milli (worst cell) = 0
server ram-hit fields: WITHDRAWN — tiering_ram_hit_p50_us, tiering_ram_hit_p99_us, tiering_ram_hit_p999_us named-absent, server says `unmeasured-iteration-clock`; the reactor's per-iteration clock cannot time a non-suspending command (ADR-0064 amendment 2026-08-08). The memory-hit half below is client-derived.
tiering_cold_resolves = 88380801
memory-hit split (client-derived, ADR-0071 D2):
  cold_frac = 8.8592% (cold_reads 521290 · cold_resolves 1042608 — re-resolve ratio 2.00×)
  mem_hit p50_us = 1087 · p99_us = 12543 · p999_us = 15359
  separation: mem_hit p99.9 15359 µs vs server cold p50 1247 µs (client tail spread 14272 µs — the truncation only separates the two modes while cold service exceeds it)
  NOT gate-eligible — separation check FAILED: derived memory-hit p99.9 15359 µs >= server cold p50 1247 µs — the two populations overlap and the quantile truncation cannot tell them apart (client tail spread 14272 µs vs cold service 1247 µs)
tripwire sqes/submit = 5.9
```
