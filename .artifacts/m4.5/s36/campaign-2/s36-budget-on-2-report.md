...
date: 1787344979 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 306.82 | PASS |
...
| S36: log + checkpoint write amplification on the RAM-resident durable shape | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; UNDEFINED until a checkpoint publishes) | 2002.00 | FAIL |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 3.54 | PASS |
...
```
device closed-loop c32 everysec ops/s=298319   p99_us=179     max_us=113452            server_cpu_pct=307   parked=3332 write_stall_p99_us=1055
device arm: io_budget_model=probed ckpts_completed=34 write_amp_milli_log_checkpoint=2002          (undefined=0) log_frame_bytes=10751729664 ckpt_bytes_total=1783758848 zero_fill_bytes=0          ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=368636]          frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997             p99_us=127     max_us=3539     server_cpu_pct=396   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=467779   p99_us=155              max_us=7280     server_cpu_pct=403  
```
