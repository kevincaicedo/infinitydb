...
date: 1787344727 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 227.78 | FAIL |
| S36: everysec pure-write throughput vs the same-session tmpfs control | >= 0.85 x (device arm ops/s / tmpfs control ops/s, same binary, same session) | 0.43 | FAIL |
| S36: log + checkpoint write amplification on the RAM-resident durable shape | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; UNDEFINED until a checkpoint publishes) | 1874.00 | FAIL |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 6.30 | PASS |
...
```
device closed-loop c32 everysec ops/s=200444   p99_us=195     max_us=419021            server_cpu_pct=228   parked=6945 write_stall_p99_us=1311
device arm: io_budget_model=probed ckpts_completed=20 write_amp_milli_log_checkpoint=1874          (undefined=0) log_frame_bytes=7251763200 ckpt_bytes_total=1044758528 zero_fill_bytes=0          ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=299710]          frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99998             p99_us=135     max_us=6301     server_cpu_pct=397   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=467144   p99_us=155              max_us=9070     server_cpu_pct=402  
```
