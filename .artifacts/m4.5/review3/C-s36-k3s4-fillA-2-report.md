...
date: 1787417129 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 271.63 | FAIL |
...
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1371.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 1.50 | PASS |
...
```
device closed-loop c32 everysec ops/s=299741   p99_us=153     max_us=424707   server_cpu_pct=272   parked=2250 write_stall_p99_us=1567
device arm: io_budget_model=probed ckpts_completed=28 write_amp_milli_log_checkpoint=1371 (undefined=0) checkpoint_over_log_milli=179 log_padding_pct=12.3 log_frame_bytes=8262561792 ckpt_bytes_total=1473040384 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=263621] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99998    p99_us=91      max_us=1495     server_cpu_pct=393   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=466566   p99_us=152     max_us=6063     server_cpu_pct=402  
```
