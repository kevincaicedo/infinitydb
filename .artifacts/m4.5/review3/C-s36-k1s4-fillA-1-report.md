...
date: 1787414957 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 1 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 275.44 | FAIL |
...
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1303.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 3.26 | PASS |
...
```
device closed-loop c32 everysec ops/s=316462   p99_us=143     max_us=435143   server_cpu_pct=275   parked=1335 write_stall_p99_us=1503
device arm: io_budget_model=probed ckpts_completed=25 write_amp_milli_log_checkpoint=1303 (undefined=0) checkpoint_over_log_milli=158 log_padding_pct=8.4 log_frame_bytes=8337068032 ckpt_bytes_total=1315315712 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=155933] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99998    p99_us=93      max_us=3258     server_cpu_pct=392   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=480769   p99_us=163     max_us=4879     server_cpu_pct=402  
```
