...
date: 1787417295 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 270.31 | FAIL |
...
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1749.00 | FAIL (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 4.77 | PASS |
...
```
device closed-loop c32 everysec ops/s=271546   p99_us=160     max_us=260610   server_cpu_pct=270   parked=2584 write_stall_p99_us=1055
device arm: io_budget_model=probed ckpts_completed=31 write_amp_milli_log_checkpoint=1749 (undefined=0) checkpoint_over_log_milli=176 log_padding_pct=29.0 log_frame_bytes=9263902720 ckpt_bytes_total=1626517504 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=217848] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99996    p99_us=330     max_us=4766     server_cpu_pct=396   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=462594   p99_us=161     max_us=5678     server_cpu_pct=402  
```
