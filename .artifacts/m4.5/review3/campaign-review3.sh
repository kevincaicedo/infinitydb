#!/usr/bin/env bash
# Review-3 campaign (2026-08-22): A = S39a rerun at K=1/4 MiB (5 ABBA pairs),
# B = K gate on the amended instrument (3 pairings × 5 rounds, rotated).
set -uo pipefail
...
# ---- Campaign A: S39a at the shipping configuration ----
A_COMMON="$COMMON --frames-in-flight 1 --staging-mib 4 --leg-idle-s 60 --fill-target-kib 16"
for spec in base-1 fillA-1 fillA-2 base-2 base-3 fillA-3 fillA-4 base-4 base-5 fillA-5; do
...
# ---- Campaign B: the K gate, three pairings rotated per round ----
