...
date: 1787417462 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 181.58 | FAIL |
| S36: everysec pure-write throughput vs the same-session tmpfs control | >= 0.85 x (device arm ops/s / tmpfs control ops/s, same binary, same session) | 0.37 | FAIL |
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 138.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1646.00 | FAIL (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 1.92 | PASS |
...
```
device closed-loop c32 everysec ops/s=170738   p99_us=153     max_us=943907   server_cpu_pct=182   parked=2698 write_stall_p99_us=975
device arm: io_budget_model=probed ckpts_completed=16 write_amp_milli_log_checkpoint=1646 (undefined=0) checkpoint_over_log_milli=138 log_padding_pct=25.7 log_frame_bytes=6069362688 ckpt_bytes_total=836485120 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=115625] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997    p99_us=108     max_us=1917     server_cpu_pct=397   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=467413   p99_us=152     max_us=9855     server_cpu_pct=402  
```
