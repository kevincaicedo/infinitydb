...
date: 1787411709 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 1 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 281.58 | FAIL |
| S36: everysec pure-write throughput vs the same-session tmpfs control | >= 0.85 x (device arm ops/s / tmpfs control ops/s, same binary, same session) | 0.66 | FAIL |
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 164.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1368.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 1.97 | PASS |
...
```
device closed-loop c32 everysec ops/s=324408   p99_us=145     max_us=484009   server_cpu_pct=282   parked=1024 write_stall_p99_us=1503
device arm: io_budget_model=probed ckpts_completed=27 write_amp_milli_log_checkpoint=1368 (undefined=0) checkpoint_over_log_milli=164 log_padding_pct=10.7 log_frame_bytes=8707022848 ckpt_bytes_total=1420201984 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=102474] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99998    p99_us=94      max_us=1974     server_cpu_pct=394   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=494481   p99_us=147     max_us=4462     server_cpu_pct=402  
```
