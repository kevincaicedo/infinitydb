...
date: 1787416963 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 283.85 | FAIL |
...
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 176.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1386.00 | PASS (informational) |
...
```
device closed-loop c32 everysec ops/s=315023   p99_us=146     max_us=275202   server_cpu_pct=284   parked=2172 write_stall_p99_us=1311
device arm: io_budget_model=probed ckpts_completed=29 write_amp_milli_log_checkpoint=1386 (undefined=0) checkpoint_over_log_milli=176 log_padding_pct=12.6 log_frame_bytes=8680857600 ckpt_bytes_total=1525657600 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=218847] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997    p99_us=90      max_us=3186     server_cpu_pct=394   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=472673   p99_us=145     max_us=5249     server_cpu_pct=402  
```
