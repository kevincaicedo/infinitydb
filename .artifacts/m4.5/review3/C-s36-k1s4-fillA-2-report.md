...
date: 1787415123 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 1 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 284.21 | FAIL |
| S36: everysec pure-write throughput vs the same-session tmpfs control | >= 0.85 x (device arm ops/s / tmpfs control ops/s, same binary, same session) | 0.69 | FAIL |
...
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1278.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 2.52 | PASS |
...
```
device closed-loop c32 everysec ops/s=344069   p99_us=142     max_us=420241   server_cpu_pct=284   parked=1037 write_stall_p99_us=1695
device arm: io_budget_model=probed ckpts_completed=26 write_amp_milli_log_checkpoint=1278 (undefined=0) checkpoint_over_log_milli=160 log_padding_pct=7.4 log_frame_bytes=8593743872 ckpt_bytes_total=1367814144 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=83329] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99998    p99_us=86      max_us=2524     server_cpu_pct=394   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=498547   p99_us=138     max_us=4172     server_cpu_pct=402  
```
