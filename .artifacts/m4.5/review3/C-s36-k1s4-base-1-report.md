...
date: 1787414791 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 1 · barrier-class fua · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 287.60 | FAIL |
...
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 159.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1416.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 1.11 | PASS |
...
```
device closed-loop c32 everysec ops/s=316129   p99_us=143     max_us=567109   server_cpu_pct=288   parked=1206 write_stall_p99_us=1279
device arm: io_budget_model=probed ckpts_completed=27 write_amp_milli_log_checkpoint=1416 (undefined=0) checkpoint_over_log_milli=159 log_padding_pct=15.0 log_frame_bytes=8951959552 ckpt_bytes_total=1419542528 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=155288] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997    p99_us=99      max_us=1110     server_cpu_pct=395   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=478659   p99_us=156     max_us=4762     server_cpu_pct=402  
```
