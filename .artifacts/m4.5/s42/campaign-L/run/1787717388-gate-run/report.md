...
date: 1787717388 (unix) · binary target/release/infinityd · cells 4 · 3 replicates · S42 stock first-boot row only · device-probe auto (the product default; no arms)
env-check: OK
tier: reference-box (binding)

notes:
- data root: /home/kcaicedo/bench-data/s42/data-L (ext4)
...
- --only-s42: every other row was skipped; their gate keys are absent
- s42 row: stock boot — `infinityd --data-dir <fresh> --cells 4 --pin-start 0` (no arm flags; the binary's `--device-probe auto` default, named on the spawn) · 3 replicates · first boot timed to ready (the probe), second boot on the same directory timed to ready (the file), then the S35 AC leg (32 conns pipeline 1, 10 s) and the read leg (64 conns × P16) on that directory · 40 s idle before every durable leg
- s42 class per replicate — the probe's verdict, then INFO's active-segment class at loading:0 → after the AC leg (ADR-0086 D4: a fresh cell upgrades at its first rotation; `flush` at loading:0 is the not-yet-zeroed first segment, not the verdict), the budget model: file fua / INFO flush→fua (4 upgrade rotations; probed) / file fua / INFO flush→fua (4 upgrade rotations; probed) / file fua / INFO flush→fua (4 upgrade rotations; probed); every boot line is in the server stderr capture when INF_GATERUN_STDERR_DIR is set
...
| S42: first boot over second boot of a fresh data directory (the in-boot probe's cost) | <= 15 s (spawn to first accepted connection, first − second, per-replicate median) | 10.13 | PASS |
| S42: every first boot reported io_properties_source=probed-at-boot on every cell (validity) | >= 1 1 = every replicate, 0 = a replicate did not | 1.00 | PASS |
| S42: every second boot reported io_properties_source=file with the identity verified (validity) | >= 1 1 = every replicate, 0 = a replicate did not | 1.00 | PASS |
| S42: the file the first boot wrote is schema 3 (identity-bound) | >= 3 io_properties_schema (INFO) | 3.00 | PASS |
| S42: always p50 over the barrier p50 at 32 conns on the directory the first boot probed (S35's bar on the stock boot) | <= 1.3 x (client p50 / barrier p50 of the class the probe chose; S35's bar) | 1.18 | PASS |
| S42: always c32 ops/s on the stock boot (informational — read against the probed arm's replicate spread) | >= 0 ops/s (32 conns closed loop, per-replicate median) | 39627.02 | PASS (informational) |
...
|---|---|
| first-boot-lifecycle | not measured by this row — S42 gates the probe lifecycle (ADR-0091 D6): the first boot's cost over the second, both provenances, the schema, and the S35 AC leg on the directory the boot probed; write amplification is S36's row |

## s42 per-replicate samples

```
rep0 first_boot_s=10.49 second_boot_s=0.13 source=probed-at-boot→file schema=3 identity=verified file_class=fua info_class=flush→fua rotations_upgrade=4 budget_model=probed c32 ops/s=39887    p50_us=719    p99_us=1915    barrier_p50_us=607   p50/barrier=1.18 frames_in_flight_max=3 acks/fsync=2.2 read c64 P16 ops/s=1637742
rep1 first_boot_s=10.26 second_boot_s=0.13 source=probed-at-boot→file schema=3 identity=verified file_class=fua info_class=flush→fua rotations_upgrade=4 budget_model=probed c32 ops/s=39543    p50_us=725    p99_us=1911    barrier_p50_us=607   p50/barrier=1.19 frames_in_flight_max=3 acks/fsync=2.2 read c64 P16 ops/s=1684870
rep2 first_boot_s=10.25 second_boot_s=0.13 source=probed-at-boot→file schema=3 identity=verified file_class=fua info_class=flush→fua rotations_upgrade=4 budget_model=probed c32 ops/s=39627    p50_us=725    p99_us=1923    barrier_p50_us=623   p50/barrier=1.16 frames_in_flight_max=3 acks/fsync=2.2 read c64 P16 ops/s=1691352
```
