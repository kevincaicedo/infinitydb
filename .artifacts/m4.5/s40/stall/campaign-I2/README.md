...
## I2 — the same row with the index-grow event (engine `339943a`)

Campaign I found (a) deterministic 12–17 ms maxima at the same seconds
of every leg with no engine event in the timeline's list, and (b) one
2.2 s device stall in a leg's first three seconds that the precedence
mislabelled `admission-park`. I2 runs the identical row with
`index_grows` (stop-and-copy flat-index doublings) on the timeline and
the corrected precedence (engine causes → index grow → device busy →
parks). Rules unchanged; both campaigns stay on the record.
