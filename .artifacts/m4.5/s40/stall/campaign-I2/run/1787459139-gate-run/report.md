...
date: 1787459139 (unix) · binary target/release/infinityd · cells 4 · 3 legs · S40 row only · frames-in-flight auto (fua 3 / flush 1) · barrier-class flush · staging-mib 4 · device-write-mbps probe-file · seal-pace off
env-check: OK
tier: reference-box (binding)

notes:
- data root: /home/kcaicedo/bench-data/s40/data-I2 (ext4)
...
- s40 attribution per leg (max's sample window): index-grow / device-busy / checkpoint
...
| S40: everysec max latency at the memtier shape, worst leg (S27 D5) | <= 50 ms (client max from the intended send instant, 32 conns pipeline 1 at 100 k offered over 1 M keys, worst of --replicates legs) | 363.46 | FAIL |
...
| S40: seconds whose per-second maximum exceeded 50 ms, summed over legs (informational) | <= 0 seconds (per-second client maxima; an isolated event reads 1, a regime reads many) | 2.00 | FAIL (informational) |
...
|---|---|
| stall-attribution | S40 stall attribution: the client's maximum is read against the 250 ms server/device sample window its send instant fell in; the attribution word is by precedence (checkpoint → rotation → manifest/truncation → zero-fill → index grow → device busy ≥ 50 % → admission park → unattributed) and is a note, never a gate |

## s40 per-leg samples

```
rep0 achieved=99999 (1.000 of offered) p50_us=66 p99_us=110 p999_us=3143 max_us=17276 max_at_s=20.24 seconds_over_50ms=0 top_seconds=[(20, 17276), (3, 13084), (5, 6294), (0, 4865), (41, 3869)] cpu_pct=393 ckpts=16 ckpt_bytes=3529310208 stall_p99_us=20479 stall_p999_us=26111 parked=0 dev_mib=11001 attribution=index-grow [index grow, stop-and-copy (+3); device: +31 MiB written, 2983 ms writing, io busy 77 % of the 252 ms window]
rep1 achieved=99999 (1.000 of offered) p50_us=67 p99_us=419 p999_us=315391 max_us=363465 max_at_s=47.95 seconds_over_50ms=2 top_seconds=[(47, 363465), (48, 325626), (30, 18836), (20, 15797), (3, 12908)] cpu_pct=392 ckpts=16 ckpt_bytes=3542949888 stall_p99_us=20991 stall_p999_us=41983 parked=42 dev_mib=10963 attribution=device-busy [admission parks (+18); device: +8 MiB written, 17843 ms writing, io busy 100 % of the 252 ms window]
rep2 achieved=100000 (1.000 of offered) p50_us=68 p99_us=119 p999_us=6671 max_us=25760 max_at_s=30.01 seconds_over_50ms=0 top_seconds=[(30, 25760), (20, 16865), (3, 12471), (5, 10823), (0, 4769)] cpu_pct=393 ckpts=16 ckpt_bytes=3526344704 stall_p99_us=19967 stall_p999_us=40959 parked=0 dev_mib=10983 attribution=checkpoint [checkpoint in flight (4→4 cells, +0 bytes), rotation (+1), frame waits (+7 barrier, +1 rotation, +0 pace); device: +38 MiB written, 10220 ms writing, io busy 103 % of the 260 ms window]
```
