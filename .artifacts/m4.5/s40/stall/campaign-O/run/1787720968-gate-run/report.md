...
date: 1787720968 (unix) · binary target/release/infinityd · cells 4 · 3 legs · S40 row only · frames-in-flight auto (fua 3 / flush 1) · barrier-class flush · staging-mib 4 · device-write-mbps probe-file · seal-pace off · flush-group-window-us 0 (off) · device-probe off
env-check: OK
tier: reference-box (binding)

notes:
- data root: /home/kcaicedo/bench-data/s40/data-O (ext4)
...
- --only-s40: every other row was skipped; their gate keys are absent
- s40 row: 4 cells · 3 legs · memtier shape on the in-house generator: 1000000 keys × 1 KiB, 32 conns, pipeline 1, everysec, 100000 offered ops/s for 60 s (latency from the intended send; a slot due on a full pipeline is skipped and counted, never sent late) · INFO + device sampled every 250 ms · 40 s idle before every leg · device stat nvme0n1
- s40 rep0: 576136 of 6000022 offered slots skipped on a full pipeline (0.0960 share) — never sent late; the achieved rate carries them
- s40 rep1: 11239 of 6000031 offered slots skipped on a full pipeline (0.0019 share) — never sent late; the achieved rate carries them
- s40 rep2: 10146 of 6000027 offered slots skipped on a full pipeline (0.0017 share) — never sent late; the achieved rate carries them
- s40 attribution candidates per leg (over the max's send-to-completion interval): checkpoint+device-busy+admission-park / checkpoint+rotation+device-busy / index-grow
...
| S40: everysec max latency at the memtier shape, worst leg (S27 D5) | <= 50 ms (client max from the intended send instant, 32 conns pipeline 1 at 100 k offered over 1 M keys, worst of --replicates legs) | 804.71 | FAIL |
| S40: achieved over offered rate, worst leg (validity — a saturated leg's max is a saturation number) | >= 0.9 x (achieved ops/s ÷ offered, min over legs) | 0.90 | PASS |
| S40: seconds whose per-second maximum exceeded 50 ms, summed over legs (informational) | <= 0 seconds (per-second client maxima; an isolated event reads 1, a regime reads many) | 10.00 | FAIL (informational) |
...
|---|---|
| stall-attribution | S40 stall attribution: the client's maximum is read against the 250 ms server/device samples spanning its actual send to its completion; every candidate class present in that interval is listed (checkpoint, rotation, manifest/truncation, zero-fill, index grow, device busy ≥ 50 %, admission park — `+`-joined) or `unattributed`; a note, never a gate. Offered slots that came due on a full pipeline are skipped, counted, and never sent late (the corrected ADR-0088 D7 generator, 2026-08-25) |

## s40 per-leg samples

```
rep0 achieved=90397 (0.904 of offered) offered=6000022 sent=5423886 skipped_pipeline_full=576136 p50_us=66 p99_us=97 p999_us=181 max_us=804709 max_intended_at_s=47.107 max_sent_at_s=47.107 max_done_at_s=47.912 seconds_over_50ms=10 top_seconds=[(47, 804709), (34, 575357), (50, 527016), (46, 519903), (53, 394522)] cpu_pct=357 ckpts=16 ckpt_bytes=3519950848 stall_p99_us=18943 stall_p999_us=47103 parked=443 dev_mib=10270 attribution=checkpoint+device-busy+admission-park [checkpoint in flight (4→4 cells, +0 bytes), admission parks (+49), frame waits (+13 barrier, +0 rotation, +0 pace); device: +35 MiB written, 98649 ms writing, io busy 90 % of the 1007 ms window]
rep1 achieved=99812 (0.998 of offered) offered=6000031 sent=5988792 skipped_pipeline_full=11239 p50_us=67 p99_us=99 p999_us=165 max_us=17507 max_intended_at_s=31.597 max_sent_at_s=31.597 max_done_at_s=31.614 seconds_over_50ms=0 top_seconds=[(31, 17507), (5, 15794), (20, 13268), (3, 7386), (0, 3329)] cpu_pct=392 ckpts=16 ckpt_bytes=3534770176 stall_p99_us=11519 stall_p999_us=27135 parked=0 dev_mib=11129 attribution=checkpoint+rotation+device-busy [checkpoint in flight (4→4 cells, +0 bytes), rotation (+4), checkpoint offers deferred (+20890), frame waits (+0 barrier, +3 rotation, +0 pace); device: +93 MiB written, 6803 ms writing, io busy 76 % of the 253 ms window]
rep2 achieved=99830 (0.998 of offered) offered=6000027 sent=5989881 skipped_pipeline_full=10146 p50_us=68 p99_us=99 p999_us=173 max_us=25738 max_intended_at_s=20.270 max_sent_at_s=20.270 max_done_at_s=20.296 seconds_over_50ms=0 top_seconds=[(20, 25738), (46, 17665), (3, 8447), (5, 6451), (58, 3379)] cpu_pct=392 ckpts=16 ckpt_bytes=3521200128 stall_p99_us=2367 stall_p999_us=40959 parked=0 dev_mib=11106 attribution=index-grow [index grow, stop-and-copy (+3); device: +27 MiB written, 39 ms writing, io busy 12 % of the 253 ms window]
```
