...
ROOT=$HOME/bench-data/s35-gate/artifacts
...
run_m2() {
  local tag=$1 k=$2 mib=$3
  echo "=== $(date -Is) m2 --only-always arm $tag (K=$k staging-mib=$mib), 40 s idle first ===" | tee -a "$LOG"
  sleep 40
  ./target/release/inf-bench gate-run m2 --only-always --reference-box \
    --barrier-class fua --frames-in-flight "$k" --staging-mib "$mib" \
    --data-root "$DATA" --artifacts-root "$ROOT/$tag-m2always" >> "$LOG" 2>&1
  echo "=== exit $? $(date -Is) ===" | tee -a "$LOG"
}
# Rotated order (k4 → k1 → k3s2): separates drive-state position from arm.
run_arm k4   4 4
run_arm k1   1 4
run_arm k3s2 3 2
run_m2  k4   4 4
