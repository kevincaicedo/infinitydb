...
date: 1787372885 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 281.36 | FAIL |
...
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 171.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1355.00 | PASS (informational) |
...
```
device closed-loop c32 everysec ops/s=312526   p99_us=151     max_us=186885   server_cpu_pct=281   parked=5851 write_stall_p99_us=1439
device arm: io_budget_model=probed ckpts_completed=28 write_amp_milli_log_checkpoint=1355 (undefined=0) checkpoint_over_log_milli=171 log_padding_pct=12.5 log_frame_bytes=8628027392 ckpt_bytes_total=1472978944 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=192628] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99996    p99_us=97      max_us=3097     server_cpu_pct=393   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=469367   p99_us=151     max_us=5578     server_cpu_pct=402  
```
