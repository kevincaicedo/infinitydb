...
date: 1787373011 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 273.68 | FAIL |
...
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1848.00 | FAIL (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 3.80 | PASS |
...
```
device closed-loop c32 everysec ops/s=269461   p99_us=171     max_us=267756   server_cpu_pct=274   parked=6334 write_stall_p99_us=1087
device arm: io_budget_model=probed ckpts_completed=32 write_amp_milli_log_checkpoint=1848 (undefined=0) checkpoint_over_log_milli=181 log_padding_pct=29.8 log_frame_bytes=9292722176 ckpt_bytes_total=1679065088 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=261075] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997    p99_us=117     max_us=3802     server_cpu_pct=396   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=466963   p99_us=155     max_us=6631     server_cpu_pct=403  
```
