...
date: 1787371303 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 274.90 | FAIL |
| S36: everysec pure-write throughput vs the same-session tmpfs control | >= 0.85 x (device arm ops/s / tmpfs control ops/s, same binary, same session) | 0.57 | FAIL |
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 177.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1774.00 | FAIL (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 3.00 | PASS |
...
```
device closed-loop c32 everysec ops/s=266264   p99_us=167     max_us=243407   server_cpu_pct=275   parked=5600 write_stall_p99_us=1023
device arm: io_budget_model=probed ckpts_completed=31 write_amp_milli_log_checkpoint=1774 (undefined=0) checkpoint_over_log_milli=177 log_padding_pct=29.7 log_frame_bytes=9192673280 ckpt_bytes_total=1626587136 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=230009] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997    p99_us=113     max_us=3000     server_cpu_pct=396   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=471059   p99_us=151     max_us=11088    server_cpu_pct=402  
```
