...
date: 1787371429 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 275.45 | FAIL |
...
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1359.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 412.93 | FAIL |
...
```
device closed-loop c32 everysec ops/s=304061   p99_us=155     max_us=308732   server_cpu_pct=275   parked=5854 write_stall_p99_us=1695
device arm: io_budget_model=probed ckpts_completed=28 write_amp_milli_log_checkpoint=1359 (undefined=0) checkpoint_over_log_milli=176 log_padding_pct=12.5 log_frame_bytes=8409051136 ckpt_bytes_total=1472999424 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=188729] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99997    p99_us=319487  max_us=412929   server_cpu_pct=380   parked=106
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=472651   p99_us=151     max_us=10820    server_cpu_pct=402  
```
