...
date: 1787370925 (unix) · binary target/release/infinityd · cells 4 · S36 row only · frames-in-flight 3 · barrier-class fua · staging-mib 2 · device-write-mbps probe-file · seal-pace off
env-check: OK
...
| S36: server CPU across the pure-write everysec leg | >= 300 % of one core (400 = four cells flat out; pre-S36 read 123–185) | 274.22 | FAIL |
...
| S36: checkpoint + MANIFEST bytes over log frame bytes (the derived trigger's 1/α bound) | <= 500 milli-x (ADR-0088 D4: interval = α × last checkpoint ⇒ checkpoint ≤ log / α; UNDEFINED until a checkpoint publishes) | 175.00 | PASS |
| S36: log + checkpoint write amplification (informational — the padding term is shape-dependent) | <= 1600 milli-x ((log frames + checkpoint + MANIFEST bytes) / encoded record bytes; log_padding_pct disclosed beside it) | 1360.00 | PASS (informational) |
| S36: everysec max latency at the comparator-matched offered rate (S27 D5) | <= 50 ms (ADR-0081 D5 bar at an offered rate; latency from the intended send instant) | 2.88 | PASS |
...
```
device closed-loop c32 everysec ops/s=304388   p99_us=151     max_us=316237   server_cpu_pct=274   parked=5599 write_stall_p99_us=1471
device arm: io_budget_model=probed ckpts_completed=28 write_amp_milli_log_checkpoint=1360 (undefined=0) checkpoint_over_log_milli=175 log_padding_pct=12.6 log_frame_bytes=8430985216 ckpt_bytes_total=1472962560 zero_fill_bytes=0 ckpt_interval_bytes(max)=268435456 deferrals[zero_fill=0 tier_flush=0 checkpoint=175860] frame_waits_pace=0
device offered-rate c32 P16 target=100000 everysec achieved ops/s=99998    p99_us=89      max_us=2885     server_cpu_pct=396   parked=0
tmpfs control (tmpfs) closed-loop c32 everysec ops/s=463237   p99_us=155     max_us=5841     server_cpu_pct=402  
```
