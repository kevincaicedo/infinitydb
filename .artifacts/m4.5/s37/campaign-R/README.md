# S37 reference campaign R — batch 32, 2026-09-11
...
Reference execution is blocked by `inf-bench env-check`: HomeLab has
nonzero lifetime thermal-throttle counters (package 72). Governors and
EPP are `performance`, turbo is disabled, and profiler access works.
`preflight-host.jsonl` records the raw host state. No counter was reset,
no environment probe was weakened, and a development smoke cannot satisfy
the reference acceptance criteria. The current NVMe is ADATA LEGEND 700,
...
This one-replicate instrument check does not accept a performance claim.
Earlier refused trials and the Q instrument smoke remain in `smoke/` and
`smoke-fixed-profile/`; each directory retains exact commands and exits.
D9's smoke completed its write and flat/read/A/A controls, but c256 B/A
throughput 0.5245, p50 B/A 1.3038 and tiered/flat throughput 0.0655 miss
the declared objectives. `smoke-fresh-db/observations.json` records the
derived ratios and their limits. Successful process exit is not D9
acceptance. The default stays off.
...
After arranging a clean reboot of the host to clear the lifetime counters,
run the existing environment setup script. Rebooting must be coordinated
with the operator; this campaign never reboots the machine. Then:

```bash
cd /home/kcaicedo/Documents/Projects/databases
./setup-infinity-benchmark-env.sh
cd infinitydb
...
set `S37_CAMPAIGN_ROOT` to a fresh path outside the worktree. Review and
copy reports, logs, profiles and environment samples into this directory,
excluding disposable database images. Record hashes and exact exit codes,
update the M4.5 plan/review ledger and claim ledger disposition, then commit
the artifact package. Never relabel an overridden environment as reference.
