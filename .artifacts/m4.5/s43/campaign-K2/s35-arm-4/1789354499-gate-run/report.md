...
date: 1789354499 (unix) · binary target/release/infinityd · cells 4 · 1 replicates · S35 row only · frames-in-flight auto (fua 3 / flush 1) · barrier-class flush · staging-mib 4 · device-write-mbps probe-file · seal-pace off · flush-group-window-us 250 · device-probe off
env-check: OK
...
- s35 4c/1c per-replicate ratios (median of 1; spread): barrier 1.572–1.572 · client p50 2.085–2.085 — client histogram 256 sub-buckets/octave (≈ 0.4 %, 2 µs at 512–1024 µs); the barrier p50 is the server's 32-sub-bucket histogram (≈ 3 %, 16 µs at that octave)
...
| S35: always p50 over the barrier p50 at 32 conns | <= 1.3 x (client p50 / barrier p50; plan AC 1.2 + 0.1-window loop overhead — K = 1 reads ~1.85) | 1.49 | FAIL |
...
| S35: N-cell vs 1-cell always client p50 ratio (informational since 2026-08-22 — carries the pipeline's seal wait at K ≥ 2) | <= 1.3 x (N-cell p50 / 1-cell p50 at 32 conns, per-replicate median on the 0.4 % client histogram; FLUSH read 1.8, FUA K = 1 1.45, K = 3 1.25–1.38 on the 3 % instrument) | 2.09 | FAIL (informational) |
...
```
rep0 4c c32  ops/s=9567     p50_us=3151   mean_us=3343   p99_us=5807    max_us=16846    barrier_p50_us=2111  barrier_p99_us=3775   p50/barrier=1.49 frames_in_flight_max=1 acks/fsync=6.4 frames=49063 parked=0 write_stall_p99_us=623 padding_pct=0.0 waits_fill=0 waits_group=17667 round_target=20
rep0 4c c256 ops/s=73136    p50_us=3719   mean_us=3497   p99_us=5887    max_us=16052    barrier_p50_us=2367  barrier_p99_us=3775   frames_in_flight_max=1 acks/fsync=50.1 frames=82939 parked=0 write_stall_p99_us=303 padding_pct=0.0 waits_fill=0 waits_group=17456 round_target=152
rep0 4c read c64 P16 ops/s=1581370  p50_us=625    p99_us=1001    p999_us=1091    nils=201015 filled=false
rep0 1c c32  ops/s=20664    p50_us=1511   mean_us=1547   p99_us=2159    max_us=15624    barrier_p50_us=1343  barrier_p99_us=1983   p50/barrier=1.13 frames_in_flight_max=1 acks/fsync=32.0 frames=7718 parked=0 write_stall_p99_us=83 padding_pct=0.0 waits_fill=0 waits_group=10 round_target=32 4c/1c: p50=2.085 barrier=1.572
```
