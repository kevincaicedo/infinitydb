...
date: 1789353657 (unix) · binary target/release/infinityd · cells 4 · 1 replicates · S35 row only · frames-in-flight auto (fua 3 / flush 1) · barrier-class flush · staging-mib 4 · device-write-mbps probe-file · seal-pace off · flush-group-window-us 0 · device-probe off
env-check: OK
...
- s35 4c/1c per-replicate ratios (median of 1; spread): barrier 1.912–1.912 · client p50 2.000–2.000 — client histogram 256 sub-buckets/octave (≈ 0.4 %, 2 µs at 512–1024 µs); the barrier p50 is the server's 32-sub-bucket histogram (≈ 3 %, 16 µs at that octave)
...
| S35: always p50 over the barrier p50 at 32 conns | <= 1.3 x (client p50 / barrier p50; plan AC 1.2 + 0.1-window loop overhead — K = 1 reads ~1.85) | 1.80 | FAIL |
| S35: N-cell vs 1-cell barrier p50 ratio (device characterization — informational since the ADR-0087 fourth amendment, 2026-08-22) | <= 1.3 x (N-cell barrier p50 / 1-cell barrier p50 at 32 conns, per-replicate median; FLUSH read ~1.8, FUA read 1.35–1.54 at every K on 08-22 — qd4 vs qd1 on the device) | 1.91 | FAIL (informational) |
| S35: N-cell vs 1-cell always client p50 ratio (informational since 2026-08-22 — carries the pipeline's seal wait at K ≥ 2) | <= 1.3 x (N-cell p50 / 1-cell p50 at 32 conns, per-replicate median on the 0.4 % client histogram; FLUSH read 1.8, FUA K = 1 1.45, K = 3 1.25–1.38 on the 3 % instrument) | 2.00 | FAIL (informational) |
...
```
rep0 4c c32  ops/s=6378     p50_us=4959   mean_us=5014   p99_us=8079    max_us=20637    barrier_p50_us=2751  barrier_p99_us=4735   p50/barrier=1.80 frames_in_flight_max=1 acks/fsync=4.3 frames=52395 parked=0 write_stall_p99_us=639 padding_pct=0.0 waits_fill=0 waits_group=0 round_target=11
rep0 4c c256 ops/s=53765    p50_us=4959   mean_us=4758   p99_us=7151    max_us=17713    barrier_p50_us=2687  barrier_p99_us=4479   frames_in_flight_max=1 acks/fsync=36.3 frames=106312 parked=0 write_stall_p99_us=639 padding_pct=0.0 waits_fill=0 waits_group=0 round_target=111
rep0 4c read c64 P16 ops/s=1624445  p50_us=603    p99_us=979     p999_us=1079    nils=683714 filled=false
rep0 1c c32  ops/s=12411    p50_us=2479   mean_us=2577   p99_us=4039    max_us=12227    barrier_p50_us=1439  barrier_p99_us=2239   p50/barrier=1.72 frames_in_flight_max=1 acks/fsync=16.0 frames=9317 parked=0 write_stall_p99_us=93 padding_pct=0.0 waits_fill=0 waits_group=0 round_target=1 4c/1c: p50=2.000 barrier=1.912
```
