...
date: 1787717009 (unix) · binary target/release/infinityd · cells 4 · 1 replicates · S35 row only · frames-in-flight auto (fua 3 / flush 1) · barrier-class flush · staging-mib 4 · device-write-mbps probe-file · seal-pace off · flush-group-window-us 250 · device-probe off
env-check: OK
...
- s35 4c/1c per-replicate ratios (median of 1; spread): barrier 1.647–1.647 · client p50 2.269–2.269 — client histogram 256 sub-buckets/octave (≈ 0.4 %, 2 µs at 512–1024 µs); the barrier p50 is the server's 32-sub-bucket histogram (≈ 3 %, 16 µs at that octave)
...
| S35: always p50 over the barrier p50 at 32 conns | <= 1.3 x (client p50 / barrier p50; plan AC 1.2 + 0.1-window loop overhead — K = 1 reads ~1.85) | 1.51 | FAIL |
| S35: N-cell vs 1-cell barrier p50 ratio (device characterization — informational since the ADR-0087 fourth amendment, 2026-08-22) | <= 1.3 x (N-cell barrier p50 / 1-cell barrier p50 at 32 conns, per-replicate median; FLUSH read ~1.8, FUA read 1.35–1.54 at every K on 08-22 — qd4 vs qd1 on the device) | 1.65 | FAIL (informational) |
| S35: N-cell vs 1-cell always client p50 ratio (informational since 2026-08-22 — carries the pipeline's seal wait at K ≥ 2) | <= 1.3 x (N-cell p50 / 1-cell p50 at 32 conns, per-replicate median on the 0.4 % client histogram; FLUSH read 1.8, FUA K = 1 1.45, K = 3 1.25–1.38 on the 3 % instrument) | 2.27 | FAIL (informational) |
...
```
rep0 4c c32  ops/s=7815     p50_us=4063   mean_us=4091   p99_us=6927    max_us=15395    barrier_p50_us=2687  barrier_p99_us=4351   p50/barrier=1.51 frames_in_flight_max=1 acks/fsync=6.4 frames=40437 parked=0 write_stall_p99_us=911 padding_pct=0.0 waits_fill=0 waits_group=14572 round_target=11
rep0 4c c256 ops/s=59767    p50_us=4559   mean_us=4279   p99_us=6879    max_us=19117    barrier_p50_us=2943  barrier_p99_us=4351   frames_in_flight_max=1 acks/fsync=49.4 frames=69426 parked=0 write_stall_p99_us=575 padding_pct=0.0 waits_fill=0 waits_group=14471 round_target=57
rep0 4c read c64 P16 ops/s=1632035  p50_us=625    p99_us=923     p999_us=1007    nils=452267
rep0 1c c32  ops/s=17430    p50_us=1791   mean_us=1835   p99_us=2463    max_us=11135    barrier_p50_us=1631  barrier_p99_us=2303   p50/barrier=1.10 frames_in_flight_max=1 acks/fsync=32.0 frames=6574 parked=0 write_stall_p99_us=81 padding_pct=0.0 waits_fill=0 waits_group=24 round_target=32 4c/1c: p50=2.269 barrier=1.647
```
