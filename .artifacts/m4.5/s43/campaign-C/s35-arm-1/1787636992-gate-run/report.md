...
date: 1787636992 (unix) · binary target/release/infinityd · cells 4 · 1 replicates · S35 row only · frames-in-flight auto (fua 3 / flush 1) · barrier-class flush · staging-mib 4 · device-write-mbps probe-file · seal-pace off · flush-group-window-us 250 · device-probe off
env-check: OK
...
- s35 4c/1c per-replicate ratios (median of 1; spread): barrier 2.001–2.001 · client p50 2.615–2.615 — client histogram 256 sub-buckets/octave (≈ 0.4 %, 2 µs at 512–1024 µs); the barrier p50 is the server's 32-sub-bucket histogram (≈ 3 %, 16 µs at that octave)
...
| S35: always p50 over the barrier p50 at 32 conns | <= 1.3 x (client p50 / barrier p50; plan AC 1.2 + 0.1-window loop overhead — K = 1 reads ~1.85) | 1.47 | FAIL |
| S35: N-cell vs 1-cell barrier p50 ratio (device characterization — informational since the ADR-0087 fourth amendment, 2026-08-22) | <= 1.3 x (N-cell barrier p50 / 1-cell barrier p50 at 32 conns, per-replicate median; FLUSH read ~1.8, FUA read 1.35–1.54 at every K on 08-22 — qd4 vs qd1 on the device) | 2.00 | FAIL (informational) |
| S35: N-cell vs 1-cell always client p50 ratio (informational since 2026-08-22 — carries the pipeline's seal wait at K ≥ 2) | <= 1.3 x (N-cell p50 / 1-cell p50 at 32 conns, per-replicate median on the 0.4 % client histogram; FLUSH read 1.8, FUA K = 1 1.45, K = 3 1.25–1.38 on the 3 % instrument) | 2.61 | FAIL (informational) |
...
```
rep0 4c c32  ops/s=7880     p50_us=3951   mean_us=4058   p99_us=6911    max_us=14999    barrier_p50_us=2687  barrier_p99_us=4351   p50/barrier=1.47 frames_in_flight_max=1 acks/fsync=6.4 frames=39900 parked=0 write_stall_p99_us=911 padding_pct=0.0 waits_fill=0 waits_group=14644 round_target=14
rep0 4c c256 ops/s=71801    p50_us=3751   mean_us=3562   p99_us=5903    max_us=18011    barrier_p50_us=2495  barrier_p99_us=4223   frames_in_flight_max=1 acks/fsync=49.3 frames=81554 parked=0 write_stall_p99_us=463 padding_pct=0.0 waits_fill=0 waits_group=17373 round_target=149
rep0 4c read c64 P16 ops/s=1593702  p50_us=659    p99_us=1087    p999_us=1203    nils=226573
rep0 1c c32  ops/s=20638    p50_us=1511   mean_us=1549   p99_us=2167    max_us=12094    barrier_p50_us=1343  barrier_p99_us=1983   p50/barrier=1.13 frames_in_flight_max=1 acks/fsync=32.0 frames=7765 parked=0 write_stall_p99_us=81 padding_pct=0.0 waits_fill=0 waits_group=22 round_target=32 4c/1c: p50=2.615 barrier=2.001
```
