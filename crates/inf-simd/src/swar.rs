//! SWAR ASCII-integer parsing — ported verbatim from `vortex-proto::swar`
//! (salvage map, master plan §24): the first eight digits parse in one
//! 64-bit SWAR pass; the proptest suite (vs `str::parse` and a scalar
//! reference) came with it. Fully safe code.
...
fn leading_digit_count(digit_values: u64) -> usize {
    ...
    if invalid == 0 { 8 } else { (invalid.trailing_zeros() as usize) >> 3 }
}
...
mod tests {
    ...
    fn scalar_parse_int(buf: &[u8]) -> Option<(i64, usize)> {
        ...
        text.parse::<i64>().ok().map(|value| (value, end)).or_else(|| {
            if negative && text == "-9223372036854775808" { Some((i64::MIN, end)) } else { None }
        })
    }
