...
fn pressure_leg(
    ...
    let dir = data_root.join(format!("inf-m2-press-{}-{label}", std::process::id()));
...
fn always_row(
    ...
    let dir = data_root.join(format!("inf-m2-alw-{}-{label}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).map_err(|e| format!("{}: {e}", dir.display()))?;
    let dir_s = dir.to_string_lossy().into_owned();
    let seg_s = PRESSURE_SEGMENT_BYTES.to_string();
...
fn everysec_row(
    ...
    let dir = data_root.join(format!("inf-m2-esec-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).map_err(|e| format!("{}: {e}", dir.display()))?;
    let dir_s = dir.to_string_lossy().into_owned();
    let seg_s = PRESSURE_SEGMENT_BYTES.to_string();
...
fn attribution_row(
    ...
    let dir = data_root.join(format!("inf-m2-attr-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).map_err(|e| format!("{}: {e}", dir.display()))?;
    let dir_s = dir.to_string_lossy().into_owned();
    let mut extra: Vec<&str> = server_extra.to_vec();
    ...
    m.note(format!(
        "attribution (durable fill leg, log domains included): sum(domains) {domains} B vs \
         VmRSS {rss} B — {divergence:.1}% divergence"
    ));
    Ok(())
}
