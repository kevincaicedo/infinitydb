...
use s37::{
    S37_DEFAULT_KEYS, S37_DEL_DEFAULT_CYCLES, S37_DEL_DEFAULT_KEYS, s37_row, s37_ticketed_del_row,
};
use s39b::{
    S39B_DEFAULT_DURATION_S, S39B_DEFAULT_SEGMENT_BYTES, s39b_recycle_row, s39b_snap, s39b_spawn,
};
    ...
    for rep in 0..replicates {
        ...
        raw.push_str(&format!(
            "rep{rep} {cells}c c{S35_CONNS_AC:<3} ops/s={:<8.0} p50_us={:<6.0} mean_us={:<6.0} \
             ...
             write_stall_p99_us={} padding_pct={:.1} waits_fill={} waits_group={} \
             round_target={}\n",
            ac.ops_per_sec,
            ac.p50_us,
            ac.mean_us,
...
fn s36_device_budget_row(
    ...
    raw.push_str(&format!(
        "device closed-loop c32 everysec ops/s={:<8.0} p99_us={:<7.0} max_us={:<8.0} \
         server_cpu_pct={:<5.0} parked={} write_stall_p99_us={}
",
        device.ops_per_sec,
        device.p99_us,
        device.max_us,
    ...
    if undefined == 0 && ckpts > 0 {
        ...
        m.note(
            "s36: write_amp_milli_log_checkpoint is UNDEFINED (no checkpoint published during \
                         the leg) — the gate reads PENDING (ADR-0060 D3), never PASS",
        );
    }
    if model_absent {
        m.note(
            "s36: the device model is ABSENT on the device arm (no schema-2 \
                 io-properties.toml              and no --device-write-mbps): background I/O \
                 unbudgeted — this is the pre-S36              baseline arm, not the budgeted one",
        );
    }
    s35_idle(idle_s, "s36 offered-rate leg");
    let paced = s36_write_leg(&server, port, cells, duration, Some(offered))?;
    raw.push_str(&format!(
        "device offered-rate c32 P16 target={offered} everysec achieved ops/s={:<8.0} \
         p99_us={:<7.0} max_us={:<8.0} server_cpu_pct={:<5.0} parked={}
",
    ...
    raw.push_str(&format!(
        "tmpfs control ({control_fstype}) closed-loop c32 everysec ops/s={:<8.0} \
         p99_us={:<7.0} max_us={:<8.0} server_cpu_pct={:<5.0}
",
    ...
    if paced.ops_per_sec < 0.9 * offered as f64 {
        m.note(format!(
            "s36: the offered-rate leg achieved {:.0} of {offered} ops/s (< 90 %) — the \
                 server              could not absorb the offered rate; its max is a saturation \
                 number, disclosed",
            paced.ops_per_sec
        ));
    }
    if !crate::gaterun::is_memory_fs(&control_fstype) {
        m.note(format!(
            "s36: WARNING — the tmpfs control root is {control_fstype}, not a memory filesystem; \
                            the 0.85× denominator is not the device-free arm it is meant to be"
        ));
    }
    m.note(format!(
        "s36 row: flat everysec, {FILL_KEYS}-key space × 1 KiB, 32 conns; device leg {}s \
             closed-loop with server CPU from /proc/<pid>/stat ({} ticks/s); write-amp scraped     \
                  at its end; offered-rate leg {duration}s at {offered} ops/s (pipeline 16, \
                  latency from          the intended send); tmpfs control {duration}s on {} \
                  ({control_fstype}, flush class);          {idle_s}s idle before every device leg",
        2 * duration,
    ...
    m.row_write_amp(
        "measured by this row as write_amp_milli_log_checkpoint (ADR-0088 D7: log frames \
             +          checkpoint + MANIFEST bytes over encoded record bytes, cell scope, boot \
             life; zero-fill          disclosed beside it) — the tier figure stays the M4 S16 \
             row's",
    );
